---
name: presentation-html-fidelity-audit-layout-discipline-d5a323
description: |
  Use this skill when Codex needs procedural guidance derived from pptx-html-fidelity-audit/references/layout-discipline.md. Recast from the original pptx-html-fidelity-audit/references/layout-discipline.md material as the presentation-html-fidelity-audit-layout-discipline-d5a323 procedure.
---

# Presentation Html Fidelity Audit Layout Discipline D5a323
...
- Original Markdown: `pptx-html-fidelity-audit/references/layout-discipline.md`
- Reformulated skill name: `presentation-html-fidelity-audit-layout-discipline-d5a323`
...
> **Procedure this file.** Skim §1-3 once to internalize the rules
...
## Constants — define once at the top of the export script
...
Avoid "solve" it by raising `CONTENT_MAX_Y`. The rail exists for a reason — content that crosses it will overlap the footer at full-screen presentation.

## Hero slides — budget centering, not cursor flow
...
## Footer is consistently pinned, never advanced

Avoid route the footer through the cursor — it has its own rail.
...
## Box height ≠ text height — but tight is better than loose
...
where `pad_pt` is 2–4 pt (≈ 0.03–0.05"). For multi-line text frames, set `text_frame.word_wrap = True` and avoid pad vertically — let the text frame's intrinsic metrics size itself.
...
## Italic preservation — only EN serif, never CJK

The single most common silent regression. HTML `<em>`, `<i>`, and inline `font-style: italic` prefer to all map to `run.font.italic = True`. But:

- **EN/Latin display copy** (Playfair Display, Origin Serif) has a real italic. Use it.
- **CJK display copy** (Noto Serif TC, Origin Han Serif) has no italic. Synthesizing produces a slanted bitmap that looks broken. Skip italic for CJK runs even if the HTML had `<em>` around the CJK text.
- **EN body copy** can use sans italic if the body family supports it; if not, swap to serif italic for the duration of the execute.
...
### 1 Cover / hero with vertical center
...
### 2 Content with pipeline (4–5 step horizontal flow)
...
`add_pipeline` internally lays out N step cards across `CONTENT_W` with `step_h` derived from the longest step's text height. Avoid fix `step_h` to a constant — let it grow to fit, and let the cursor's overflow guard catch problems.

### 3 Two-column comparison / concern cards
...
## Common pitfalls and how the discipline catches them
