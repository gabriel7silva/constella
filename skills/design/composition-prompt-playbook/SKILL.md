---
name: composition-prompt-playbook
description: |
  Use this skill when Codex needs parse a structured design brief written in I-Lang protocol format into a concrete design spec. Eliminates ambiguity from vague requests like "make it professional" by requiring explicit dimensions: palette, typography, layout, mood, density, and constraints. Trigger keywords: "design brief", "produce a design brief", "ilang brief", "structured brief". Recast from the original design-brief/SKILL.md material as the composition-prompt-playbook procedure.
---

# Composition Prompt Playbook
...
- Original Markdown: `design-brief/SKILL.md`
- Reformulated skill name: `composition-prompt-playbook`
...
Every design brief needs to resolve these 8 dimensions. If any are missing from the input, select sensible defaults using the rules in Section 2.2.

The values listed below form a closed vocabulary. Only values in this table have concrete token mappings in Section 2.1. If the user provides a value not listed here, the agent needs to prompt for clarification rather than guessing.
...
### 1 Symbolic → concrete token resolution

Each symbolic value maps to concrete design tokens. The agent needs to resolve these before writing DESIGN.md:
...
Symbolic values not in this table are not valid. If the user provides an unrecognized value (e.g., `palette=ocean_blue`), the agent needs to prompt for clarification: "I avoid recognize `palette=ocean_blue`. Did you mean `navy_and_white`, `monochrome_dark`, `light_clean`, or `earth_tones`?"

### 2 Default resolution rules
...
This skill generates a new DESIGN.md from scratch based on the resolved brief dimensions. If a DESIGN.md already exists in the working directory, the agent prefer to ask the user whether to overwrite or skip.

Produce a DESIGN.md following Open Design's 9-section convention. All color hex values, font stacks, and spacing values needs to come from the resolved tokens in Section 2.1 — do not invent values outside the resolution table.
...
## Compose brief-preview.html

Produce a single HTML file that visually renders the resolved design tokens. The preview needs to contain these 4 sections in order:
...
4. **Component preview** — Render 2–3 live components (a primary button, a card with title/body, a text input) using the resolved tokens. These prefer to be functional HTML/CSS, not screenshots.

Style the preview itself with the resolved design system tokens (background color, font, spacing). The preview prefer to look like a design system documentation page.

## Report unspecified dimensions
