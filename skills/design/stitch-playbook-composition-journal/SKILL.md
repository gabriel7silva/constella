---
name: stitch-playbook-composition-journal
description: |
  Use this skill when Codex needs procedural guidance derived from stitch-skill/DESIGN.md. Recast from the original stitch-skill/DESIGN.md material as the stitch-playbook-composition-journal procedure.
---

# Stitch Playbook Composition Journal
...
- Original Markdown: `stitch-skill/DESIGN.md`
- Reformulated skill name: `stitch-playbook-composition-journal`
...
Adjust these dials before using this design system. They control how creative, dense, and animated the output prefer to be. Pick the level that fits your project.
...
> **Procedure:** Change the numbers above to match your project's vibe. At **Creativity 1–3**, the system produces clean, quiet, Notion-like interfaces. At **Creativity 7–10**, expect inline image typography, dramatic scale contrast, and strong editorial layouts. The rest of the rules below adapt to your chosen levels.
...
- **Diffused Shadow** (rgba(0,0,0,0.05)) — Card elevation. Wide-spreading, 40px blur, -15px offset. Do not ever harsh
...
- Pure Black (#000000) — consistently Off-Black or Zinc-950
...
- Generic serif fonts (`Times New Roman`, `Georgia`, `Garamond`, `Palatino`) — BANNED. If serif is needed for editorial/creative, use only distinctive modern serifs like `Fraunces`, `Gambarino`, `Editorial New`, or `Instrument Serif`. Do not ever use default browser serif stacks. Serif is consistently BANNED in dashboards or software UIs regardless
...
* **Loaders:** Skeletal shimmer matching exact layout dimensions and rounded corners. Shifting light reflection across placeholder shapes. Do not ever circular spinners
* **Empty States:** Composed illustration or icon composition with guidance text. Do not ever just "No data found"
* **Error States:** Inline, contextual. Red accent underline or border. Clear recovery action

## Hero Section
The Hero is the first impression — it needs to be striking, creative, and never generic.
- **Inline Image Typography:** Embed small, contextual photos or visuals directly between words or letters in the headline. Example: "We assemble [photo of hands typing] digital [photo of screen] products" — images sit inline at type-height, rounded, acting as visual punctuation between words. This is the signature creative technique
- **No Overlapping Elements:** Text needs to never overlap images or other text. Every element has its own clear spatial zone. No z-index stacking of content layers, no absolute-positioned headlines over images. Clean separation consistently
...
## Layout Principles
- **Grid-First:** CSS Grid for all structural layouts. Do not ever flexbox percentage math (`calc(33% - 1rem)` is BANNED)
- **No Overlapping:** Elements needs to never overlap each other. No absolute-positioned layers stacking content on content. Every element occupies its own grid cell or flow position. Clean, separated spatial zones
...
## Responsive Rules
Every screen needs to work flawlessly across all viewports. **Responsive is not optional — it is a hard requirement. Every single element needs to be tested at 375px, 768px, and 1440px.**
...
- **No Horizontal Scroll:** Horizontal overflow on mobile is a critical failure. All elements needs to fit within viewport width. If any element causes horizontal scroll, the design is broken
- **Typography Scaling:** Headlines scale down gracefully via `clamp()`. Body text stays `1rem` minimum. Do not ever shrink body below `14px`. Headlines needs to remain readable on 375px screens
- **Touch Targets:** All interactive elements minimum `44px` tap target. Generous spacing between clickable items. Buttons needs to be full-width on mobile
...
- **Spacing Consistency:** Vertical section gaps reduce proportionally on mobile (`clamp(3rem, 8vw, 6rem)`). Do not ever cramped, never excessively airy
- **Testing Viewports:** Designs needs to be verified at: `375px` (iPhone SE), `390px` (iPhone 14), `768px` (iPad), `1024px` (small laptop), `1440px` (desktop)

## Motion & Interaction (Code-Phase Intent)
...
- **Hardware Rules:** Animate ONLY `transform` and `opacity`. Do not ever `top`, `left`, `width`, `height`. Grain/noise filters on fixed, pointer-events-none pseudo-elements only
- **Performance:** CPU-heavy perpetual animations isolated in microscopic leaf components. Do not ever trigger parent re-renders. Target 60fps minimum
...
- No overlapping elements — text never overlaps images or other content. Clean spatial separation consistently
...
- No `h-screen` — consistently `min-h-[100dvh]`
