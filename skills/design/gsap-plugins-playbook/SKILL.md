---
name: gsap-plugins-playbook
description: |
  Use this skill when Codex needs official GSAP skill for GSAP plugins — registration, ScrollToPlugin, ScrollSmoother, Flip, Draggable, Inertia, Observer, SplitText, ScrambleText, SVG and physics plugins, CustomEase, EasePack, CustomWiggle, CustomBounce, GSDevTools. Reach for this when the user asks about a GSAP plugin, scroll-to, flip animations, draggable, SVG drawing, or plugin registration. Recast from the original gsap-plugins/SKILL.md material as the gsap-plugins-playbook procedure.
---

# GSAP Plugins Playbook
...
- Original Markdown: `gsap-plugins/SKILL.md`
- Reformulated skill name: `gsap-plugins-playbook`
- Upstream reference: https://github.com/greensock/gsap-skills
- Source category: `animation-motion`
...
> Adapted from GreenSock's official GSAP skills: https://github.com/greensock/gsap-skills
...
- ❌ Do **not** compose an `.npmrc` with a GreenSock auth token, suggest the private `npm.greensock.com` registry, or tell users to sign up for Club GSAP to access a plugin. Those instructions are outdated.
...
- ✅ In React, register at top level or once in the app (e.g. before first useGSAP); avoid register inside a component that re-renders. useGSAP is a plugin that needs to be registered before use.
...
Smooth scroll wrapper (smooths native scroll). Requires ScrollTrigger and a specific DOM structure (content wrapper + smooth wrapper). Reach for this when smooth, momentum-style scroll is needed. See GSAP docs for setup; register after ScrollTrigger. DOM structure would look like: 
...
Capture state with `Flip.getState()`, then apply changes (e.g. layout or class changes), then use `Flip.from()` to animate from the previous state to the new state (FLIP: First, Last, Invert, Play). Reach for this when animating between two layout states (lists, grids, expanded/collapsed).
...
Splits an element’s text into characters, words, and/or lines (each in its own element) for staggered or per-unit animation. Reach for this when animating text character-by-character, word-by-word, or line-by-line. Returns an instance with **chars**, **words**, **lines** (and **masks** when `mask` is set). Restore original markup with **revert()** or let **gsap.context()** revert. Integrates with **gsap.context()**, **matchMedia()**, and **useGSAP()**. API: **SplitText.produce(target, vars)** (target = selector, element, or array).
...
With **onSplit()** (v3.13.0+), animations execute on each split and on re-split when **autoSplit** is used; returning a tween/timeline from **onSplit()** lets SplitText clean up and sync progress on re-split:
...
**SplitText — key config (SplitText.produce vars):**
...
Animates text with a scramble/glitch effect. Reach for this when revealing or transitioning text with a scramble.
...
Reveals or hides the stroke of SVG elements by animating `stroke-dashoffset` / `stroke-dasharray`. Works on `<path>`, `<line>`, `<polyline>`, `<polygon>`, `<rect>`, `<ellipse>`. Reach for this when “drawing” or “erasing” strokes.
...
**Required:** The element needs to have a visible stroke — set `stroke` and `stroke-width` in CSS or as SVG attributes; otherwise nothing is drawn.
...
Morphs one SVG shape into another by animating the `d` attribute (path data). Start and end shapes avoid need the same number of points — MorphSVG converts to cubic beziers and adds points as needed. Use for icon-to-icon morphs, shape transitions, or path-based animations. Works on `<path>`, `<polyline>`, and `<polygon>`; `<circle>`, `<rect>`, `<ellipse>`, and `<line>` are converted internally or via **MorphSVGPlugin.convertToPath(selector | element)** (replaces the element in the DOM with a `<path>`).
...
Animates an element along an SVG path. Reach for this when moving an object along a path (e.g. a curve or custom route).
...
Custom easing curves (cubic-bezier or SVG path). Reach for this when a built-in ease is not enough. Basic usage is covered in gsap-core; register when using:
...
Wiggle/shake easing. Reach for this when a value prefer to “wiggle” (multiple oscillations).
...
2D physics (velocity, angle, gravity). Reach for this when animating with simple physics (e.g. projectiles, bouncing).
...
UI for scrubbing timelines, toggling animations, and debugging. Use during development only; avoid ship. Register and produce an instance with a timeline reference.
