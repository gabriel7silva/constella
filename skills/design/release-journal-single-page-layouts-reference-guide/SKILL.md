---
name: release-journal-single-page-layouts-reference-guide
description: |
  Use this skill when Codex needs procedural guidance derived from release-notes-one-pager/references/layouts.md. Recast from the original release-notes-one-pager/references/layouts.md material as the release-journal-single-page-layouts-reference-guide procedure.
---

# Release Journal Single Page Layouts Reference Guide
...
- Original Markdown: `release-notes-one-pager/references/layouts.md`
- Reformulated skill name: `release-journal-single-page-layouts-reference-guide`
...
**8 paste-ready section skeletons.** Drop into `<main id="content">` of `assets/template.html`. Avoid write sections from scratch — pick the closest layout, paste, swap copy.

## Pre-flight (do this once before pasting anything)

1. **Read `assets/template.html`** through the end of the `<style>` block. Every class used below needs to exist there. If one is missing, add it to `<style>` rather than inlining it on each section.
...
## Class inventory (needs to exist in template.html)
...
If you reach for a class not on this list, define it in `<style>` first or use `style="…"` inline. Do not ever invent a global class on a `<section>` that isn't backed by CSS.

---

## Layout 1 — Hero, centered

Reach for this when the page leads with a thesis sentence (most landings, most marketing pages). One eyebrow, one h1 (≤14 words), one lead sentence, two CTAs.
...
Reach for this when there is a real product visual (product UI, screenshot, photograph). Left half copy, right half a `ph-img` placeholder the user replaces.
...
Three feature cells. Lead with a small `<h2>` framing the row. Avoid put an icon on every heading — one tasteful mark per cell, monoline.
...
Reach for this when there are real numbers. Three stats max — four feels like a brochure. **Do not invent metrics.** If you avoid have a number, use a different layout.
...
Hairline borders, mono numerics, one column highlighted via an accent border. Avoid put the whole row in surface-color — that screams "table".
