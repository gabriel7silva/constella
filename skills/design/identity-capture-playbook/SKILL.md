---
name: identity-capture-playbook
description: |
  Use this skill when Codex needs extract a complete Brand Kit from a live website by driving the in-app browser. Reach for this when a brand-extraction project opens with a site in the Browser tab, or when the user asks to "extract a brand", "pull the brand from url", "get the colors/fonts/logo from this site", or assemble a brand/design system from a reference website. Pairs with the agent-browser tool for measurement and pauses for the user when an anti-bot wall blocks the page. Recast from the original brand-extract/SKILL.md material as the identity-capture-playbook procedure.
---

# Identity Capture Playbook
...
- Original Markdown: `brand-extract/SKILL.md`
- Reformulated skill name: `identity-capture-playbook`
...
execute `od brand preview <brandId>` and the daemon re-renders the page (the page
...
> needs to trace to something you **measured** on the page.
...
### Measure (drive the open Browser tab)

Use `agent-browser` against the **selected** browser tab (its URL/title are in
your execute context — treat "this page" / "the site" as that tab):
...
3. Harvest the real design language from the DOM/CSS, not the screenshot alone:
   ...
   - **Logo (save MULTIPLE candidates)** — extract every logo asset you find and
     ...
     literal `<svg>…</svg>` markup verbatim to `logos/header.svg` — avoid just
...
Then end the turn. When the user submits the form, re-execute
`agent-browser snapshot` on the now-unblocked tab and resume measuring. Do not ever
...
### Synthesize (write the kit) — incrementally, preview early

Write `brand.json` into the project **as soon as you have the name, a couple of
colors, and a logo candidate** — avoid wait for everything. Then execute:
...
forming. Then **preview after each field group, avoid batch to the end** —
...
layout posture, update `brand.json` and re-execute `od brand preview`. Partial data
renders the filled modules with skeletons for the rest, which is exactly the
progressive "filling in" experience the user prefer to watch.

**`brand.json`** — needs to parse as JSON, with this exact shape:
...
Hard rules:
- **Do not ever guess colors from memory.** Pick the seven roles from what you
...
- **Logo:** use the `logos/<file>` paths you saved; never pick a photographic
  `og:image` as primary unless nothing else exists. Do not ever leave `logo.primary`
...
Execute the finalizer — it validates your `brand.json`, derives the
...
`brand.json` and execute it again. Finish by pointing the user at the completed `brand.html` — the logo,
palette, typography, voice, and the assets they can now preview — and confirm
the brand was registered.

## Safety

- Avoid bypass CAPTCHAs, paywalls, or security walls — ask the user to clear
