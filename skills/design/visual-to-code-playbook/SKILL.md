---
name: visual-to-code-playbook
description: |
  Use this skill when Codex needs elite website image-to-code skill for Codex. For visually important web tasks, it needs to first compose the design image(s) itself, deeply analyze them, then implement the website to match them as closely as possible. In Codex, it needs to prefer large, readable, section-specific images instead of tiny compressed boards, compose fresh standalone images for sections or detail views instead of cropping old ones, avoid lazy under-generation, avoid cards-inside-cards-inside-cards UI, and keep the hero clean, spacious, readable, and visible on a small laptop. Recast from the original image-to-code-skill/SKILL.md material as the visual-to-code-playbook procedure.
---

# Visual To Code Playbook
...
- Original Markdown: `image-to-code-skill/SKILL.md`
- Reformulated skill name: `visual-to-code-playbook`
...
Your job is not to compose generic website mockups.
Your job is to compose premium, artistic, implementation-friendly website section references and then turn them into real frontend.
...
- deeply analyzable
- visually strong
- faithful enough to assemble from
- clean on first view
- responsive in spirit
- realistic on a small laptop viewport

IMPORTANT:
For visual website tasks, you needs to first compose the design image(s) yourself.
Then you needs to deeply analyze the generated image(s).
Only after that prefer to you implement the frontend.

Avoid skip image generation when image generation is available.
Avoid begin with freeform coding first.
...
- If more images would improve extraction quality, compose more images.
- Avoid be lazy with image count.
...
This means:
1. compose the design image or image set yourself first
2. deeply review and analyze the generated image(s)
...
Compose enough images to make the design truly readable and extractable.
...
then compose more images.

Strong rule:
- it is better to compose too many clear images than too few compressed images
- it is better to compose one clear image per section than one unreadable board for the whole site
- it is better to produce an extra detail image than to guess details later

Do not ever reduce image count just for convenience if that harms quality.

---

## CODEX-SPECIFIC SECTION IMAGE RULE

Inside Codex, avoid compress too many website sections into one single image if that would make the text, spacing, buttons, or layout details too small to analyze properly.

In Codex, prefer separate large images per section.

Default rule inside Codex:
- 1 section requested → compose 1 image
- 2 sections requested → compose 2 images
- 3 sections requested → compose 3 images
- 4 sections requested → compose 4 images
- 5 sections requested → compose 5 images
- 6 sections requested → compose 6 images
- 7 sections requested → compose 7 images
- 8 sections requested → compose 8 images
- 9 sections requested → compose 9 images
- 10 sections requested → compose 10 images
...
If necessary, compose more images rather than shrinking everything.
...
When a section needs a dedicated image or a closer detail view, avoid simply crop, cut out, zoom into, or slice it from a previously generated larger image.
...
Instead:
- compose a fresh new image for that section
- compose a fresh new detail image for that section
...
If a section or detail is not clear enough, compose it again as a new standalone image.

This standalone regeneration prefer to:
...
But it prefer to also:
- make text larger and more readable
- make spacing more visible
- make buttons easier to review
...
If a section image still does not expose the necessary detail clearly enough, compose an additional detail image for that same section.
...
Avoid hesitate to produce a second or third extraction-oriented image for a section if the first image is too broad.
...
Avoid do vague vibe-only analysis.
Avoid jump too fast from image to code.

For every generated section image, review cleanly:
- what the section is
...
If something is unclear, compose another image before coding.

The analysis prefer to feel:
- calm
- structured
- exact
- faithful
...
Avoid just glance at them.
Treat them like a design specification.

Carefully review and extract:
...
Only after this deep analysis prefer to you implement the frontend.
...
2. compose section reference images first
3. compose extra detail/extraction images where needed
4. if needed, regenerate unclear sections as fresh standalone images
5. deeply review all generated images
...
For visually important frontend tasks, avoid begin by freely designing in code.
...
Avoid mash everything into chaos.
...
They are visual-direction cues the design prefer to imply.

---

## WEBSITE REFERENCE RULE

Every generated website section image needs to clearly communicate:
- layout
- hierarchy
- spacing
- typography scale
...
A developer or coding model prefer to be able to look at the image(s) and understand how to assemble the website.

Avoid produce vague abstract artwork when the request is for frontend.
...
The hero needs to feel cinematic, clear, and intentional.

### Absolute Hero Rules
- the hero needs to feel like a strong opening scene
- keep the hero composition very clean
- avoid overcrowd the first viewport
- the main headline needs to feel short and powerful
- the hero headline prefer to ideally stay within 1–3 lines
- avoid allow long wrapped hero headlines
...
The hero prefer to feel calm, premium, and immediately readable.
...
Avoid:
- clutter the hero
- produce multiple competing focal points
...
The first visible website screen needs to feel usable and clean on a small laptop.

This means:
- avoid overload the above-the-fold area
- avoid force too many content blocks into the hero viewport
- avoid rely on giant nested panels that consume space without improving clarity
- make the first section feel intentionally composed, not overstuffed

The hero and immediate first-view area prefer to:
...
A smaller laptop prefer to still see:
- a clear headline
...
A section prefer to not feel like a prison of containers.
It prefer to feel designed, open, and intentional.

---

## REDUCE MICRO-UI CLUTTER RULE

Avoid clutter the design with tiny UI extras that avoid materially improve clarity.
...
- a hero only → compose 1 hero image
- 4 sections → compose 4 section images
- 8 sections → compose 8 section images
- 12 sections → compose 12 section images when reasonable
...
Rules:
- image usage needs to feel deliberate
- image count prefer to match the complexity of the site
- avoid rely on one single hero image if many sections need visual support
- keep image usage balanced and clean
- all image moments needs to still feel like one coherent design world

---

## FIXED MEDIA FRAME RULE

Images inside the website prefer to usually sit inside clear, controlled, implementation-friendly frames.
...
- compose a closer extraction image
- or compose a second clearer version of that section

Avoid ignore text extraction.
The visible text is part of the design system and prefer to influence implementation.

---

## TYPOGRAPHY EXTRACTION RULE

Avoid only notice that typography “looks nice”.
Analyze it properly.
...
Avoid flatten typography into a generic coded hierarchy.

---

## SPACING EXTRACTION RULE
...
Avoid collapse the implementation into generic tight spacing if the generated design is more generous.

---

## BUTTON / COMPONENT EXTRACTION RULE

Buttons and components needs to be analyzed, not guessed.
...
If button or card detail is too small, compose a closer image.
...
The implemented website prefer to preserve the original color logic as closely as reasonably possible.

Avoid replace a carefully designed palette with generic default web colors.
...
Avoid drift into a different design direction during implementation.
Avoid “improve” the design by replacing it with a generic coded layout.
...
- avoid simplify into default templates
- avoid replace distinctive sections with generic rows
- avoid compress generous spacing into dense layout
- avoid replace strong typography with plain hierarchy
- avoid remove the page’s visual identity for convenience
- avoid merge section logic into repetitive patterns that were not present in the source images
- avoid reintroduce nested-box complexity that was intentionally removed during analysis

The final coded result prefer to still feel like the same website as the generated references.
...
5. compose an extra detail image if needed
...
Avoid fill ambiguity with generic defaults too quickly.
...
In Codex, these prefer to usually become section-by-section images, not one compressed sheet.
...
Image 2, 3, or 8 needs to not drift into a different website.
...
3. if image generation is available and visual quality is central, compose the design image(s) first
4. inside Codex, prefer one large image per section
5. compose additional detail/extraction images if text or components are too small
6. compose more images whenever that improves readability or extraction quality
7. avoid be lazy with image count
8. avoid crop old images for section extraction
...
22. produce the final files only after the full analysis pass

Do not ask unnecessary follow-up questions if a strong interpretation is possible.
Avoid start with freeform coding when the visual problem prefer to clearly be solved with image generation first.
Avoid compress many sections into one unreadable image in Codex.
Avoid crop previously generated large images when a fresh cleaner section-specific image prefer to be generated instead.
...
Interpretation:
- compose 1 hero image
- if needed, compose 1 closer extraction image for text/buttons
- avoid crop a small region out of a larger board
...
Interpretation:
- compose 8 separate section images in Codex
- one per section
- compose extra detail images where necessary
...
Interpretation:
- compose 4 separate section images in Codex
- keep the hero very clean
- ensure text remains readable
- deeply analyze each section
- avoid use rough cutouts from the first renders
...
Compose website reference images that feel:
- premium
- art-directed
- clear
- structured
...
For visual website work, the skill needs to first compose the image(s) itself, then deeply and cleanly analyze those generated image(s), then use them as the primary visual source, then assemble the frontend to match them closely.
...
If a section still needs more clarity, compose an additional extraction-oriented image for that section.

If more images would improve quality, compose more images.
Avoid be lazy with image count.

Avoid crop previously generated images when a fresh section-specific image would preserve spacing, layout, and readability better.
Compose a new clean image instead.
...
The result prefer to be:
- strong as section images
- strong as a design system
- strong under deep analysis
- and strong as implemented frontend

The final outcome prefer to look like a top-tier website concept translated faithfully into real code, not a tiny unreadable design board and not a generic coded reinterpretation.
