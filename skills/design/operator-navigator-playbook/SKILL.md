---
name: operator-navigator-playbook
description: |
  Use this skill when Codex needs browser automation CLI for AI agents. Reach for this when the user needs to review, test, or automate browser behavior: navigating pages, filling forms, clicking buttons, taking screenshots, extracting page data, reading selected Open Design browser-tab context, testing web apps, dogfooding Open Design previews, QA, bug hunts, or reviewing app quality. Prefer local Open Design preview URLs unless the user explicitly asks for external browsing. Recast from the original agent-browser/SKILL.md material as the operator-navigator-playbook procedure.
---

# Operator Navigator Playbook
...
- Original Markdown: `agent-browser/SKILL.md`
- Reformulated skill name: `operator-navigator-playbook`
- Upstream reference: https://github.com/vercel-labs/agent-browser/blob/main/skills/agent-browser/SKILL.md
...
Use `agent-browser` for local Open Design preview validation: review rendered
...
When the execute prompt contains selected workspace context, prefer the selected
...
Avoid replace the CLI with ad hoc browser scripts.

## Context Hygiene

Do not ever print full upstream guides into chat or tool output. Save them to temp
...
the user is building from the reference. Avoid paste full page HTML or large
asset dumps into chat; summarize the relevant selectors, tokens, URLs, and
screenshots.

## CDP Startup Contract

`agent-browser` needs to attach to an existing CDP endpoint. Do not ever execute
`agent-browser open` before `agent-browser connect`; doing so can make the CLI
auto-launch Chrome and re-enter the crash path.

Avoid execute Open Design's own daemon CLI as a browser automation tool. Commands
...
Lightpanda is optional. Avoid try `--engine lightpanda` unless
...
With the Open Design preview at `http://127.0.0.1:17573/`, execute:
...
6. If the execute prompt includes a selected browser workspace item, open or focus
...
- Avoid submit forms, send messages, change permissions, produce keys, upload
  files, delete data, purchase anything, or transmit sensitive information
  without explicit user confirmation at action time.
- Avoid bypass CAPTCHAs, paywalls, security interstitials, or age checks.
- Avoid use persistent authenticated browser state unless the user explicitly
...
Load these only when directly needed, and consistently redirect to temp files:
