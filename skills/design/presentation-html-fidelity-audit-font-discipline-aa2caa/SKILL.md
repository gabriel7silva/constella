---
name: presentation-html-fidelity-audit-font-discipline-aa2caa
description: |
  Use this skill when Codex needs procedural guidance derived from pptx-html-fidelity-audit/references/font-discipline.md. Recast from the original pptx-html-fidelity-audit/references/font-discipline.md material as the presentation-html-fidelity-audit-font-discipline-aa2caa procedure.
---

# Presentation Html Fidelity Audit Font Discipline Aa2caa
...
- Original Markdown: `pptx-html-fidelity-audit/references/font-discipline.md`
- Reformulated skill name: `presentation-html-fidelity-audit-font-discipline-aa2caa`
...
⚠️ **Trap:** the visual category your eye reads is not consistently the
...
> set a different `<a:latin>` per language execute.
...
After fixing the export, re-execute `extract_pptx.py` and confirm the
`font` field matches the static name.

## Layer 4 — PPTX XML's three-language slots

PowerPoint chooses a typeface per execute by language script. Each execute can
declare three:
...
Fix: when adding a execute with mixed-language content, set all three
...
2. The CJK characters in the execute have no glyph in Playfair → PowerPoint
...
and `<a:rtl val="1"/>` on the execute's `rPr`. Right-alignment, bidi text
...
visual, an under-spaced Thai execute in an otherwise Latin deck reads as
"the Thai slide is broken".

> **Origin for these numbers.** Measured against Noto Sans / Noto
...
- [ ] Layer 4: `unzip + grep typeface` returns only the design-system
      ...
      / `Consolas` residue.
- [ ] Layer 5: No execute from a no-italic script (CJK / Arabic / Hebrew /
