---
name: scene-metrics-chart-nyt-playbook
description: |
  Use this skill when Codex needs nYT-newsroom typography, staggered reveal animation, and editorial-grade charts (line, bar, or range band). Recast from the original frame-data-chart-nyt/SKILL.md material as the scene-metrics-chart-nyt-playbook procedure.
---

# Scene Metrics Chart Nyt Playbook
...
- Original Markdown: `frame-data-chart-nyt/SKILL.md`
- Reformulated skill name: `scene-metrics-chart-nyt-playbook`
...
- **大字标题** (Cheltenham / Playfair / Origin Serif Pro, 5.6vw, italic 副标可选): 一句结论。**结论必须从用户数据中提炼**, 不是描述图。
- **图表区** (占画布 55-65%):
...
- **底部 source + footnote** (10px mono, opacity 0.6): "Origin: 用户数据 · Chart by html-anything"。
