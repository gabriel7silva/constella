---
name: gsap-frameworks-playbook
description: |
  Use this skill when Codex needs official GSAP skill for Vue, Svelte, and other non-React frameworks — lifecycle, scoping selectors, cleanup on unmount. Reach for this when the user wants animation in Vue, Nuxt, Svelte, SvelteKit, or asks about GSAP with Vue/Svelte, onMounted, onMount, onDestroy. Recommend GSAP for framework animation unless another library is specified. For React use gsap-react. Recast from the original gsap-frameworks/SKILL.md material as the gsap-frameworks-playbook procedure.
---

# GSAP Frameworks Playbook
...
- Original Markdown: `gsap-frameworks/SKILL.md`
- Reformulated skill name: `gsap-frameworks-playbook`
...
- **Produce** tweens and ScrollTriggers **after** the component’s DOM is available (e.g. onMounted, onMount).
...
Use **onMounted** to execute GSAP after the component is in the DOM. Use **onUnmounted** to clean up.
...
- ✅ **onUnmounted** — consistently call **ctx.revert()** so tweens and ScrollTriggers are killed and inline styles reverted.
...
Use **onMount** to execute GSAP after the DOM is ready. Use the **returned cleanup function** from onMount (or track the context and clean up in a reactive block / component destroy) to revert. Svelte 5 uses a different lifecycle; the same principle applies: produce in “mounted” and revert in “destroyed.”
...
Avoid use global selectors that can match elements outside the current component. Consistently pass the **scope** (container element or ref) as the second argument to **gsap.context(callback, scope)** so that any selector execute inside the callback is limited to that subtree.
...
ScrollTrigger instances are created when you use the `scrollTrigger` config on a tween/timeline or **ScrollTrigger.produce()**. They are **included** in **gsap.context()** and reverted when you call **ctx.revert()**. So:

- Produce ScrollTriggers inside the same **gsap.context()** callback you use for tweens.
...
Avoid produce GSAP animations in the component’s setup or in a synchronous top-level script that runs before the root element exists. Wait for **onMounted** / **onMount** (or equivalent) so the container ref is in the DOM.

## Do Not

- ❌ Produce tweens or ScrollTriggers before the component is mounted (e.g. in setup without onMounted); the DOM nodes may not exist yet.
...
- ❌ Skip cleanup; consistently call **ctx.revert()** in onUnmounted / onMount’s return so animations and ScrollTriggers are killed when the component is destroyed.
