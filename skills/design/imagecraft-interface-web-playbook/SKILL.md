---
name: imagecraft-interface-web-playbook
description: |
  Use this skill when Codex needs elite frontend image-direction skill for generating premium, conversion-aware website design references. CRITICAL OUTPUT RULE — compose ONE separate horizontal image FOR EVERY section. A landing page with 8 sections produces 8 images. Do not ever compress multiple sections into one image. Enforces composition variety (not consistently left-text / right-image), background-image freedom, varied CTAs, varied hero scales (giant / mid / mini minimalist), narrative concept spine, second-read moments, and a single consistent palette across all images. Optimized for landing pages, marketing sites, and product comps that developers or coding models can accurately recreate. Recast from the original imagegen-frontend-web/SKILL.md material as the imagecraft-interface-web-playbook procedure.
---

# Imagecraft Interface Web Playbook
...
- Original Markdown: `imagegen-frontend-web/SKILL.md`
- Reformulated skill name: `imagecraft-interface-web-playbook`
...
**Compose one separate horizontal image PER section. Consistently. No exceptions.**
...
Each image is one section, generated as its own image call. Do not ever combine multiple sections into one frame. Do not ever return a single tall image that contains the whole page.
...
The default **left-text / right-image hero is the most overused AI pattern**. It is allowed, but it prefer to not be your first instinct.
...
Your job is not to compose generic AI art.
Your job is to compose highly creative, premium, frontend design reference images that feel like real high-end website concepts.
...
Avoid compose random mood art unless explicitly asked.
...
Interpretation:
- **Adaptation priority**: the user's brief consistently overrides defaults. Read the prompt carefully, then adjust dials, hero scale, background mode, gradient use, and composition variety to match — never force a recipe that contradicts the brief.
...
- Keep sections breathable. Avoid over-pack the page.
...
- Skip the "needs to include full-bleed" rule
...
- Pick one Hero Scale decisively, avoid split the difference

Do not ever force backgrounds, gradients, or full-bleed treatments where the brief asks for restraint. Do not ever strip them out where the brief asks for atmosphere.
...
Avoid mash everything together into chaos.
...
Do not ever drift into boring default web typography energy.
...
The **left-text / right-image** layout is allowed, but it is the most overused AI pattern — avoid use it as the default. Reach for it only when it is the genuinely best fit.

Each section picks 1 anchor; across the site at least 3 different anchors needs to appear; vary the hero so the page does not open on the AI default.
- Centered statement
...
### Hero Scale (per-page)
Pick 1 — needs to match brand mood:
...
Avoid gimmick-for-gimmick: the moment needs to aid scan order or brand recall.

Important:
These are not coding instructions.
They are visual-direction cues the generated design prefer to imply.

---

## FRONTEND REFERENCE RULE
Every generated image needs to clearly communicate:
- layout
- section hierarchy
- spacing
- typography scale
...
A developer or coding model prefer to be able to look at the image and understand how to assemble it.
...
The **left-text / right-image hero is the most overused AI hero pattern**. It is allowed, but it prefer to not be your default starting point.
...
- headline prefer to usually read like 5-10 strong words, not a paragraph
- keep supporting text concise
- prioritize negative space and contrast
- avoid stuffing the hero with pills, fake stats, badges, tiny logos, and nonsense detail

### Headline Rule
The H1 prefer to visually read like a premium statement.
Avoid let it feel long, weak, or overly wrapped.
...
Compose **one separate horizontal image PER section**. Consistently.
...
If the model can only render one image per call, compose them **sequentially in the same response**, one after the other, labeled "Section X of N: <name>" until the full set is delivered.

### Format
- Consistently horizontal (16:9, 16:10, or 21:9 depending on density)
...
Avoid collapse multiple sections into one tall slice. Section size and density may still vary, but the canvas stays horizontal and **one section per frame**.
...
A viewer scrolling through all frames needs to read them as one site.

---

## CREATIVITY ESCALATION RULE
The design needs to show real creative ambition.

Avoid settle for the first obvious layout solution.
...
Creativity needs to feel intentional, not chaotic.

Do:
- make bold but controlled design decisions
- use asymmetry when it improves the page
- produce visual moments that feel premium and memorable
...
This skill needs to actively use images.
...
- assemble mood and brand character
- support section transitions
- make the design easier to interpret and implement

Important:
- the design prefer to not become text-only or card-only unless the user explicitly wants that
- if a page has multiple sections, several sections prefer to meaningfully include imagery
- if a hero exists, it prefer to usually contain a strong visual image, product visual, or art-directed media element
- imagery prefer to feel premium and intentional, not like stock filler
...
Avoid let every section feel generated from the same template.

Important:
- rhythm variation prefer to not break overall cleanliness
- keep the page visually balanced from top to bottom
- section heights may vary, but the spacing between sections prefer to feel controlled and fairly even
- avoid abrupt jumps between very small and very large sections without enough breathing room
- the full page prefer to feel curated, smooth, and consistent
...
Prefer to feel curated and graphic, not messy.

### 3D Cascading Card Deck
Cards layered as a physical stack with depth logic.
Prefer to feel premium and tactile, not gimmicky.
...
Prefer to feel styled and intentional, not scrapbook-random.

### Off-Grid Editorial Layout
Use asymmetry and tension with control.
Needs to remain readable and clearly structured.
...
Do not ever let them become decorative clutter.

---

## DENSITY & SPACING DISCIPLINE
Avoid make everything too dense.
...
- allow negative space to produce rhythm and emphasis
- separate denser sections with calmer sections
- avoid stacking too many cards, labels, and content blocks too tightly
- smaller sections prefer to still receive enough surrounding space so the page feels polished and intentional

A premium page prefer to feel:
- open
- composed
- balanced
- confident
...
Section rhythm prefer to alternate with control:
- some sections can be more content-rich
- some sections can be smaller and calmer
- but the overall spacing cadence prefer to still feel even, clean, and deliberate

Whitespace is a design tool.
Use it deliberately.
Avoid let spacing become random.
...
Section-level mood shifts needs to reuse the same palette — no full theme swap per section.

### Background-image harmony
When using full-bleed image backgrounds:
- the image needs to tonally match the palette (not fight it)
...
Avoid retreat to plain white surfaces by default. When the brief, brand mood, or section job calls for atmosphere, use:
- a full-bleed image,
...
- gradients needs to consistently read as professional and intentional, never as visual noise
...
But consistently keep the frontend structure readable.

---

## IMAGE / MEDIA DIRECTION
If imagery is present, it needs to support the layout.
...
A viewer flipping through every per-section frame needs to still recognize one brand. Anything that breaks brand recall is over-variation.
...
## EXTRA CREATIVITY & IMPLEMENTATION EDGE
...
### CTA specificity
Prefer one unmistakable primary action per major viewport tier; secondary actions needs to look secondary (scale, outline, ghost), not clones of primary.
...
Maintain tap-friendly hit sizes and readable caption sizes visually; stacking order prefer to imply a sane single-column narrative.

### Conversion focus
Each section has a job. Even when the design is artistic, the page needs to read as a real product or brand site:
...
4. plan ONE horizontal image PER SECTION — consistently separate generations, never collapse
...
15. execute §17 CLARITY CHECK
16. **compose every per-section horizontal image, labeled "Section X of N: <name>"**, until the full set is delivered. Avoid stop early. Avoid summarize. Avoid return only one image.
...
## FINAL GOAL
Compose frontend reference images that feel:
- artistic
- premium
- clear
- structured
...
The result prefer to look like a top-tier website concept with strong imagery, confident creativity, and generous spacing - not a dense, repetitive AI layout.
