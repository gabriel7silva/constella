---
name: presentation-html-fidelity-audit-playbook
description: |
  Use this skill when Codex needs audit a python-pptx export against its source HTML deck, identify layout/content drift (footer overflow, cropped content, missing italic/em, lost styling, off-rhythm spacing), and re-export with strict footer-rail + cursor-flow layout discipline. Use this skill whenever the user has a .pptx that was generated from an HTML slide deck and asks to compare/audit/verify/fix the export — including phrases like "compare ppt with html", "fidelity audit", "fix the pptx", "ppt is cut off", "footer overlap", "italic missing in pptx", "re-export the deck", "pptx-html-fidelity-audit", or any case where a python-pptx → HTML round-trip needs verification or repair. Also trigger when the user shows you a deck.html and a deck.pptx side by side and is debugging visual differences. Recast from the original pptx-html-fidelity-audit/SKILL.md material as the presentation-html-fidelity-audit-playbook procedure.
---

# Presentation Html Fidelity Audit Playbook
...
- Original Markdown: `pptx-html-fidelity-audit/SKILL.md`
- Reformulated skill name: `presentation-html-fidelity-audit-playbook`
...
If the user only has *one* of those two artifacts, this skill doesn't apply yet — first compose the missing one, or ask the user to provide it.
...
The audit is five steps. Avoid skip any of them — the discipline only works if the audit produces a real list of issues to drive the re-export. A fix-without-audit pass tends to leave half the issues alive.

### Step 1 — Extract ground truth from the PPTX

Execute `scripts/extract_pptx.py <path-to.pptx> > pptx_dump.json`. The script walks every shape on every slide and dumps text, position (`top` / `left`), size (`width` / `height`), and per-execute typography (font name, size pt, bold, italic, color). This is the *actual* state of the export — avoid trust the export script's intent, trust the dump.
...
### Step 3 — Assemble the audit table
...
- 🔴 **critical** — content cropped, text invisible, footer overlap, off-canvas. Needs to fix.
- 🟠 **high** — content visible but visual hierarchy broken, no breathing room, hero not centered. Prefer to fix.
- 🟡 **medium** — italic/em missing, font fallback wrong, color drift. Fix in this pass.
- 🟢 **low** — minor spacing/alignment, sub-pixel offsets. Note but avoid block.
...
For each slide, instantiate `Cursor(MARGIN_TOP)` and `take(height)` each block in reading order. The slide refuses to render if any block would cross the rail, so overflows become loud assemble errors instead of silent visual bugs.
...
When walking HTML, detect `<em>` / `<i>` / inline style `font-style: italic` and pass `italic=True`. Use the EN serif face (Playfair Display, Origin Serif, or fallback Georgia) for italic display copy — the CJK serif typically has no italic and looks broken if you try to italicize it.
...
After writing the new `.pptx`, execute `scripts/verify_layout.py <path-to.pptx>`. The script:
...
Zero violations is the gate for "this re-export is shippable". Avoid claim the audit is fixed without running the verifier — the human eye misses 1–2 mm overflow at zoom-out, the script doesn't.
...
- `scripts/extract_pptx.py` — dump every shape on every slide as JSON. Execute before the audit. **Important:** also execute on the *original* export to compare, and on the *re-exported* one to confirm.
...
- **Trusting the original export script's intent.** Consistently execute the extractor against the actual file. Drift between intent and reality is the bug.
