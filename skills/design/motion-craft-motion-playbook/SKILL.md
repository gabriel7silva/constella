---
name: motion-craft-motion-playbook
description: |
  Use this skill when Codex needs motion-design follow-up skill inspired by Emil Kowalski's animation guidance. Use after an interface exists to add tasteful micro-interactions, state transitions, and page motion with product-grade restraint. Recast from the original emilkowalski-motion/SKILL.md material as the motion-craft-motion-playbook procedure.
---

# Motion Craft Motion Playbook
...
- Original Markdown: `emilkowalski-motion/SKILL.md`
- Reformulated skill name: `motion-craft-motion-playbook`
- Upstream reference: https://emilkowal.ski/skill
...
1. Review the current HTML, component, or selected page element before adding animation.
...
- Default UI transitions prefer to feel quick and useful: 140-220ms for most controls.
- Larger page reveals can be slower, but needs to not block reading.
- Avoid endless decorative loops unless they communicate status or progress.
- Avoid add custom cursors, noisy particle effects, or motion that competes with content.
...
- Consistently clean up observers, timers, and animation instances.
