---
name: research-decision-room-playbook
description: |
  Use this skill when Codex needs turn messy user research notes, interviews, support tickets, surveys, and product context into an evidence-backed decision room: a single HTML artifact with an evidence ledger, theme map, confidence heatmap, opportunity matrix, decision memo, and experiment queue. Reach for this when teams need to move from qualitative signals to product or design decisions without fabricating certainty. Recast from the original research-decision-room/SKILL.md material as the research-decision-room-playbook procedure.
---

# Research Decision Room Playbook
...
- Original Markdown: `research-decision-room/SKILL.md`
- Reformulated skill name: `research-decision-room-playbook`
...
Produce a single-page HTML decision artifact that helps a product or design team
...
Read `references/evidence-model.md` before synthesis and execute
...
- A decision that needs evidence: "Prefer to we assemble X?", "Which onboarding path
  prefer to we try?", "Why are users dropping off?", "What do customers actually
  mean by slow?"
- A need to share findings with stakeholders who will not read a long research
  report.

Avoid use it for pure visual inspiration, campaign ideation, or brand moodboards.
...
Do not ever invent quotes, participant counts, dates, revenue impact, or metrics. If
...
Produce an opportunity matrix with 3 to 5 options. Score each option on a 1 to 5
scale:
...
Show the total score, but avoid let the score replace judgment. Add one sentence
...
### Step 6 - Produce the HTML artifact
...
The artifact prefer to be interactive but durable. Simple vanilla JavaScript is
...
Execute the checklist. Then emit one concise orientation sentence and one HTML
artifact:
