---
name: composition-taste-interface-playbook
description: |
  Use this skill when Codex needs anti-slop frontend skill for landing pages, portfolios, and redesigns. The agent reads the brief, infers the right design direction, and ships interfaces that do not look templated. Real design systems when applicable, audit-first on redesigns, strict pre-flight check. Recast from the original taste-skill/SKILL.md material as the composition-taste-interface-playbook procedure.
---

# Composition Taste Interface Playbook
...
- Original Markdown: `taste-skill/SKILL.md`
- Reformulated skill name: `composition-taste-interface-playbook`
...
### B Output a one-line "Design Read" before generating
...
### C If the brief is ambiguous, ask one question, do not guess
Ask exactly **one** clarifying question - never a multi-question dump - and only when the design read genuinely diverges. Example: *"Should this feel closer to Linear-clean or Awwwards-experimental?"*

If you can confidently infer from context, **do not ask**. Just declare the design read and proceed.

### D Anti-Default Discipline
...
### A Dial Inference (design read → dial values)
...
### A When to reach for a real design system (use official packages)
...
For these directions, there is **no single official package**. Assemble with native CSS + Tailwind + a maintained component library. Be honest in code comments about what is borrowed inspiration vs. official material.
...
* **Fonts:** Consistently use `next/font` (Next.js) or self-host with `@font-face` + `font-display: swap`. Do not ever link Google Fonts via `<link>` in production.
...
* **NEVER hand-roll SVG icons.** If a glyph is missing, install a second library or compose from primitives - avoid draw icon paths from scratch.
...
### E Responsiveness & Layout Mechanics
* Standardize breakpoints (`sm 640`, `md 768`, `lg 1024`, `xl 1280`, `2xl 1536`).
* Contain page layouts using `max-w-[1400px] mx-auto` or `max-w-7xl`.
* **Viewport Stability:** NEVER use `h-screen` for full-height Hero sections. ALWAYS use `min-h-[100dvh]` to prevent layout jumping on mobile (iOS Safari address bar).
* **Grid over Flex-Math:** NEVER use complex flexbox percentage math (`w-[calc(33%-1rem)]`). ALWAYS use CSS Grid (`grid grid-cols-1 md:grid-cols-3 gap-6`).

### F Dependency Verification (mandatory)
Before importing ANY 3rd-party library, check `package.json`. If the package is missing, output the install command first. **Do not ever** assume a library exists.
...
* **One palette per project.** Avoid fluctuate between warm and cool grays within the same project.
...
* **PREMIUM-CONSUMER PALETTE BAN (mandatory, second-most-recurring AI-tell):**
  ...
  * **Default alternatives (rotate, avoid reuse):**
  ...
  * **Palette-rotation rule:** if the previous premium-consumer project you generated used the beige+brass family, this one MUST use a different family. Avoid ship the same warm-craft palette twice in a row.
...
LLMs default to "static successful state only." Consistently implement full cycles:
...
* **CTA BUTTON WRAP BAN (mandatory):** Button text MUST fit on one line at desktop. If a label like "VIEW SELECTED WORK" wraps to 2 or 3 lines, the button is broken. Fix by EITHER shortening the label (3 words max for primary CTAs, ideally 1-2) OR widening the button (avoid artificially constrain `max-width` on CTAs). Wrapped CTAs at desktop are a Pre-Flight Fail.
...
### 6 Data & Form Patterns
* Label ABOVE input. Helper text optional but present in markup. Error text BELOW input. Standard `gap-2` for input blocks.
* No placeholder-as-label. Ever.

### 7 Layout Discipline (Hard Rules. Failing any of these is shipping broken work)

* **Hero MUST fit in the initial viewport.** Headline max 2 lines on desktop, subtext max **20 words** AND max 3-4 lines, CTAs visible without scroll. If the copy is too long: reduce font scale OR cut copy. If you cannot describe the value-prop in 20 words of subtext, the value-prop is unclear, not the rule too tight. Do not ever let the hero overflow and force scroll to find the CTA.
* **Hero font-scale discipline.** Plan font size and image size *together*. If the hero asset is large and the headline is more than 6 words, avoid start at `text-7xl/text-8xl`. Default sensible range: `text-4xl md:text-5xl lg:text-6xl` for most heroes; `text-6xl md:text-7xl` only when the headline is 3-5 words. A 4-line hero headline is consistently a font-size error, never a copy-length error.
...
* **"Used by" / "Trusted by" logo wall belongs UNDER the hero, never inside it.** The hero is for the value prop and primary CTA. The logo wall is a separate section directly below. Avoid stuff trust logos into the same flex row as the hero copy.
* **Navigation MUST render on a single line on desktop.** If items avoid fit at `lg` (1024px), condense labels, drop secondary items, or move to a hamburger. A two-line nav at desktop is broken design.
* **Navigation height cap: 80px max desktop, default 64-72px.** No huge "agency" nav bars that eat 15% of the viewport.
* **Bento grids MUST have rhythm, not one-sided repetition.** Avoid stack 6 left-image / right-text rows. Vary the composition: alternate full-width feature rows, asymmetric tile sizes, vertical breaks.
* **BENTO CELL COUNT RULE (mandatory):** A bento grid has EXACTLY as many cells as you have content for. 3 items → 3 cells (1+2 split, or 2+1, or asymmetric trio). 5 items → 5 cells (2+3, 3+2, hero+4, etc.). If your grid has an empty cell in the middle or at the end, you planned wrong. Re-shape the grid; avoid paste a blank tile.
* **Section-Layout-Repetition Ban.** Once you use a layout family for a section (e.g., 3-column-image-cards, full-width-quote, split-text-image), that family can appear at most ONCE on the page. "Selected commissions" needs to not look like "What we do." A landing page with 8 sections needs to use at least 4 different layout families.
...
* **Mobile collapse needs to be explicit per section.** For every multi-column layout, declare the `< 768px` fallback in the same component. No "it'll work, Tailwind handles it" assumptions.
...
* **Origin: Simple Icons** (`https://cdn.simpleicons.org/{slug}/ffffff` for any color, or `simple-icons` npm package). Covers most known brands.
* **Alternative: devicon** for tech-stack logos (`@svgr/cli` or CDN).
* **Make-up the brand name? Then make-up an SVG mark too.** Compose a simple monogram (one letter in a circle, two-letter ligature, abstract glyph) rendered as an inline `<svg>` matching the page style. Plain text wordmarks for invented brand names look generic.
* **Consistently** ensure logos render in both light and dark mode (white-on-dark, black-on-light, or single-color theme variable).
...
* **Default content shape per section:** short headline (≤ 8 words) + short sub-paragraph (≤ 25 words) + one visual asset OR one CTA. Anything more needs to be justified by the section's job.
...
* **Long lists need a different UI component, not a longer list.** Default `<ul>` with bullets / `divide-y` rows is the lazy choice. If you have > 5 items, reach for one of these instead:
  ...
  - Marquee for "lots-of-things-that-avoid-need-individual-attention"
...
* **COPY SELF-AUDIT (mandatory before ship):** Before declaring any task done, re-read every visible string on the page (headlines, subheads, eyebrows, button labels, body copy, captions, alt text, footer text, error messages). Flag any string that is:
  ...
  - **Sounds like AI hallucination** (cute-but-wrong wordplay, forced metaphors that avoid track, "elegant nothing" phrases)
...
* **Fake-precise numbers are flagged.** Numbers like `92%`, `4.1×`, `48k`, `5.8 mm`, `13.4 lb` either:
  ...
  - Are AI-invented spec aesthetics - banned. Avoid fake engineering precision the brand doesn't claim.
* **One copy register per page.** Do not mix technical mono ("47 tasks · 0.6 ctx-switches/day"), editorial prose, and marketing punch in the same composition unless the brand voice explicitly calls for it.

### 10 Quotes & Testimonials

* **Max 3 lines** of quote body. Do not ever 6. If the original quote is longer → cut it. A landing-page quote is a snippet, not the full review.
...
* Attribution: name + role + (optionally) company. Do not ever name only ("- Sarah").
* Quote marks: use real typographic quotes ( " " ) or none at all. Not straight ASCII ( " ).

### 11 Page Theme Lock (Light / Dark Mode Consistency)

The page has ONE theme. Sections avoid invert.

* If the page is dark mode, ALL sections are dark mode. No light-mode-warm-paper section sandwiched between dark sections (or vice versa). The user needs to not feel they walked into a different website mid-scroll.
...
* When using a design system with built-in theming (Radix Themes, shadcn/ui with `<Theme>`), set the theme ONCE in `layout.tsx` or the page root. Avoid let individual sections override.
...
* **Magnetic Micro-physics:** Reach for this when `MOTION_INTENSITY > 5` AND the brief reads premium / playful / agency. Implement EXCLUSIVELY with Motion's `useMotionValue` / `useTransform` outside the React render cycle. Do not ever `useState`. See Section 3.B.
* **Perpetual Micro-Interactions** (Pulse, Typewriter, Float, Shimmer, Carousel): Reach for this when `MOTION_INTENSITY > 5` AND the section actively benefits from motion (status indicators, live feeds, AI-feel). **Not every card needs an infinite loop.** If a section is informational, leave it still. Apply Spring Physics (`type: "spring", stiffness: 100, damping: 20`) - no linear easing.
* **"Motion claimed, motion shown."** If `MOTION_INTENSITY > 4`, the page needs to actually move: entry transitions on hero, scroll-reveal on key sections, hover physics on CTAs, at minimum. A static page that claims `MOTION_INTENSITY: 7` is broken. Conversely, if you cannot ship working motion in the available scope, drop the dial to 3 and ship a clean static page. Do not ever half-assemble motion that breaks (cut-off ScrollTriggers, jumpy enters, missing cleanups).
...
* **GSAP Sticky-Stack Pattern (when scroll-stack is used).** A "card stack on scroll" needs to be a REAL sticky-stack, not a sequential reveal list. See Section 5.A below for the canonical code skeleton. Common failure: trigger fires halfway through scroll instead of pinning at viewport top. Fix: `start: "top top"` not `start: "top center"` or `"top 80%"`.
...
### C Scroll-Reveal Stagger - Canonical Skeleton (lighter alternative)
...
* **Layout Transitions:** Use Motion's `layout` and `layoutId` props for visible state changes (re-ordering lists, expanding modals, shared elements between routes). Avoid wrap static content in `layout` props "for safety" - it costs measurement work.
...
* Animate ONLY `transform` and `opacity`. Do not ever animate `top`, `left`, `width`, `height`.
* Use `will-change: transform` sparingly - only on elements that will actually animate.

### B Reduced Motion (mandatory)
...
### C Dark Mode (mandatory for any consumer-facing page)
* Design for **both modes from the start**. Do not ever ship light-only or dark-only without explicit user instruction.
* Use Tailwind `dark:` variant OR CSS variables for tokens. Pick one strategy per project.
* **Avoid prescribe specific dark-mode colors here.** The brief decides. Maintain visual hierarchy, brand identity, and WCAG AA contrast (AAA for body) across both modes.
...
* **LCP** < 2.5s. Hero image needs to be `next/image priority` or preloaded.
* **INP** < 200ms. Heavy work off main thread.
* **CLS** < 0.1. Reserve space for images, fonts, embeds.
* Execute Lighthouse before declaring a page done.
...
Dual-mode by default. Do not ever assume light-only unless the brief is print-emulating editorial.
...
* **Hierarchy parity** - visual hierarchy that works in light needs to work in dark. If a CTA pops in light, it pops in dark.
* **Brand fidelity** - primary brand color stays recognisable. Avoid desaturate the brand into a dark mode.
...
Open the page in both modes during development. Avoid ship a page you've only seen in one mode.
...
### D Content & Data ("Jane Doe" Effect)
...
* **NO div-based fake screenshots.** Do not ever assemble a fake product UI out of `<div>` rectangles to simulate a screenshot. Use real images, generated images, or skip the preview.
...
### F Production-Test Tells (banned outright)
...
* **NO section-number eyebrows.** `00 / INDEX`, `001 · Capabilities`, `002 · Featured commission`, `06 · how it works`, `05 · The honest table` - banned. Eyebrows prefer to name the topic in plain language, not enumerate.
* **NO `01 / 4`-style pagination on images or bento tiles.** If the user can count, they avoid need the label.
...
* **NO `<br>`-broken-and-italicized headlines** as a default "design move." "for thirty\<br\>*years.*" type splits. Headlines prefer to read naturally first, get clever only when the brief demands it.
...
* **NO generic step labels.** "Stage 1 / Stage 2 / Stage 3", "Step 1 / Step 2 / Step 3", "Phase 01 / Phase 02 / Phase 03", "Pass One / Pass Two / Pass Three". Banned. The actual step content is the label. If you needs to show progression, use the verb-noun directly ("Install", "Configure", "Ship") not "Stage 1: Install".
...
* **NO "Reservation 412 of 800"-style live-stock counters** as decoration. Only if the brief is explicitly a limited-execute waitlist with real data.

**Decoration text strips**
* **NO decoration text strip at hero bottom.** Patterns like `BRAND. MOTION. SPATIAL.`, `TYPE / FORM / MOTION`, `DESIGN · BUILD · SHIP`, `ESTD. 2018 · LISBON · BRAND. MOTION. SPATIAL.` as a small mono-caps strip across the bottom of the hero are an agency-portfolio cliché. Banned by default. Only acceptable when the strip carries real, navigable links (sticky bottom nav) or real status info (cookie banner, assemble info on a docs site).
* **NO floating top-right sub-text in section headings.** Pattern: section has a giant left-aligned headline; in the top-right corner of the same section header there is a small explainer paragraph floating with no clear alignment to anything else. That floater is the Tell. Either put the sub-text directly under the headline, or assemble a clean 2-column header (left: headline, right: aligned body), but not a tiny corner paragraph.
...
### G EM-DASH BAN (the single most-violated Tell)
...
If your output contains a single `—` or `–` anywhere visible to the user, the output fails the Pre-Flight Check and needs to be rewritten.
...
## REFERENCE VOCABULARY (Pattern Names the Agent Prefer to Know)

This is a vocabulary, not a library. The agent prefer to KNOW these pattern names to communicate about them, design with them in mind, and reach for them when the design read calls for them. **Implementations and code sketches live in the Block Library (Section 12), which is populated iteratively.**
...
### A Detect the Mode (first action)
...
If ambiguous, ask **once**: *"Prefer to this redesign preserve the existing brand, or are we starting visually from scratch?"*
...
### C Preservation Rules
* **Avoid change information architecture** unless asked. Keep page slugs, anchor IDs, primary nav labels stable for SEO and muscle memory.
...
* **Honor existing accessibility wins.** Avoid regress focus states, alt text, keyboard nav, contrast.
* **Respect existing analytics events.** Avoid rename buttons, form fields, section IDs that downstream tracking depends on.

### D Modernisation Levers (priority order)
...
### E Decision Tree: Targeted Evolution vs Full Redesign
* IA, content, and SEO sound → **targeted evolution** (Levers 1-4). ~70% of value at ~40% of risk.
* Visual debt is structural (broken IA, no design system, broken mobile) → **full redesign** with strict content preservation.
* Brand itself is changing → **greenfield**.

### F What Do not ever Changes Silently
Do not ever modify without explicit user approval:
...
## THE BLOCK LIBRARY (Contract - Implementations Land Here Iteratively)
...
**Status:** schema defined here. Blocks will be added iteratively. Avoid freelance new blocks without following this schema.
...
### D Block-Library Discipline
* One block per file. No multi-block files.
* Every block needs to work standalone (drop it into a page, it renders).
* Every block needs to pass the Pre-Flight Check (Section 14).
...
Execute this matrix before outputting code. This is the last filter.

**THIS IS NOT OPTIONAL. Execute every box. If any box fails, the output is not done.**
...
# APPENDICES - Real Origin-Backed Reference Material
...
**Important:** `prefers-reduced-transparency` has uneven browser support; test it. Consistently provide enough contrast even without blur.
