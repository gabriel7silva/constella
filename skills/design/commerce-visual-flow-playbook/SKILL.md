---
name: commerce-visual-flow-playbook
description: |
  Use this skill when Codex needs reference-product ecommerce image workflow for generating a compact set of product-faithful main, feature, and lifestyle images from real product reference photos. V1 requires uploaded product imagery and intentionally defers brief-only concept generation and platform-specific batch exports. Recast from the original ecommerce-image-workflow/SKILL.md material as the commerce-visual-flow-playbook procedure.
---

# Commerce Visual Flow Playbook
...
- Original Markdown: `ecommerce-image-workflow/SKILL.md`
- Reformulated skill name: `commerce-visual-flow-playbook`
...
Produce a compact ecommerce image set from real product reference imagery.
...
photo, ask for one and stop. Avoid produce a brief-only concept product in
this version.
...
By default, compose three ecommerce-ready image assets for one product:
...
Review the reference image and write a short internal identity lock:
...
- What needs to not change.

Use these anchors in every generation prompt.

### Step 2 - Assemble a three-slot shot plan

Produce a compact shot plan before dispatch:
...
Every prompt needs to include this product fidelity instruction near the top:
...
Use the unified Open Design media dispatcher. Avoid call provider APIs or
custom model commands directly.

For each slot, execute the standard compose/wait loop:
...
The final line needs to be JSON with `{"file": {"name": "...", ...}}`.
...
After generation, produce a project file named `image-manifest.json`:
...
Produce a simple single-file HTML gallery that:
...
Avoid emit an `<artifact>` tag.

## Hard rules

- V1 requires real product reference imagery. No brief-only concept products.
- One product per execute.
- Default to exactly three slots: main, feature, lifestyle.
- Preserve the product; avoid redesign it.
- Do not invent claims, certifications, measurements, ingredients, or
  performance data.
- Use `"$OD_NODE_BIN" "$OD_BIN" media generate`; avoid call provider APIs
  directly.
- Consistently produce `image-manifest.json` after generation.
- Execute `references/checklist.md` before handoff.
