...
**Current stable release: `v0.7.1`** — a security-triage patch on top of 0.7.0: a Critical SSRF fix (the
dev-server frameability probe), git argument-injection hardening on GitHub import, an http(s)-only scheme
guard on the preview address bars, plus defense-in-depth on org deletion and workspace regex search. Everything
from 0.7.0 below still applies.
