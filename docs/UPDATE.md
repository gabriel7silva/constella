...
**Current stable release: `v0.6.1`** — **Fire agent** (remove a hired agent, with an in-app confirmation),
a per-agent **Effort** (reasoning) control, and **real model IDs** fetched live from the CLI where the
binary supports it. See what changed in the [Changelog](../CHANGELOG.md) (the Update module also shows it
inline as “What's new”).
