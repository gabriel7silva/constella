...
**Current stable release: `v0.7.2`** — fixes a bug where messages to Grace in the Design module could silently
stop sending after an SSE reconnect. Includes the **`v0.7.1`** security triage (a Critical SSRF fix, git
argument-injection hardening on GitHub import, an http(s)-only scheme guard on the preview address bars, plus
defense-in-depth on org deletion and workspace regex search) and everything in **`v0.7.0`** below.

**`v0.7.0`** — a **Stop button** in every chat (and a global **Stop All** on Pulse),
