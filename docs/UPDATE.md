...
**Current stable release: `v0.9.1`** — patches **every** actionable dependency advisory (a crafted ~120-byte ZIP
could crash the process through `adm-zip`; Next.js proxy-bypass, SSRF and Server-Actions DoS; a `brace-expansion`
input that stalls the event loop for minutes) and makes **Claude Opus 5** the recommended default model.

Also in the 0.9 / 0.8 line: **`v0.9.0`** lets you hire **Grok Build**, xAI's terminal coding agent, as a real
agent; **`v0.8.0`** collapses Grace into **one chat** — her DM now builds the Design canvas live, with the
module's separate chat column gone. Two fixes before that are worth calling out if you run a server:
**`v0.7.3`** got agents replying at all over plain HTTP on a LAN/Tailscale IP (total silence before), and
**`v0.7.4`** fixed **inbound Telegram** messages (plus the 24/7 tick and file-sync) never reaching Constella.
...
### v0.9.1 — Patched dependencies + Claude Opus 5
- **Every actionable dependency advisory is patched** (7 high, 5 moderate). `adm-zip` → 0.6.0: a ~120-byte
  crafted ZIP that declared a ~4 GB uncompressed size forced a huge allocation *before* the checksum was
  verified, crashing the process — and that path is reachable wherever Constella opens an archive (restoring
  archived work, extracting a local model). `next` → 16.2.11 (proxy/middleware bypass, Server-Actions DoS,
  SSRF on custom servers), plus `postcss`, `sharp` and `brace-expansion` (a ~90-byte input could stall the
  event loop for minutes).
- **Claude Opus 5 is the recommended default model.** The `opus` alias now resolves to Opus 5, so the pickers
  read "Claude Opus 5" and Anthropic's default preference leads with it. Your existing agents keep the model
  they were hired with, and a new company still spreads models by role (cheap ones for the light agents) so
  nobody wakes up paying Opus prices for the whole staff.

### v0.9.0 — Grok Build (xAI) agents
- **Hire Grok Build**, xAI's terminal-native coding agent, like any other agent. It's auto-detected if the
  `grok` CLI is installed, shows up in the provider catalog and the model pickers, and edits files + runs
  shell like the other agentic CLIs. Install it with `curl -fsSL https://x.ai/cli/install.sh | bash`
  (`irm https://x.ai/cli/install.ps1 | iex` on Windows) and sign in with `grok`. Note this is the local agent
  CLI — different from the existing xAI / Grok HTTP provider.

### v0.8.0 — One Grace chat: her DM builds the canvas
- **The Design module no longer has its own chat column.** You talk to Grace in her DM, and that DM builds and
  updates the canvas live. Before, there were two separate Grace conversations doing nearly the same thing.
  Everything the Design chat could do moved over unchanged — streaming replies, suggestion chips, image
  attach/paste/drop, attaching a canvas element, the context donut, Stop. Opening `/design` pops her DM open
  beside the canvas.
- **Grace's replies no longer show up as `(couldn't respond: …)`** — her actual answer was being wrapped in
  that error text when a long run hit its turn/time limit.

### v0.7.4 — The worker can reach its own endpoints
- **Messages sent to the Telegram bot never reached Constella** (replies going out worked fine). The
  background worker's own requests were being redirected to the login page, which silently broke inbound
  Telegram, the 24/7 tick, and the disk→database file-sync — while still logging a misleading success. Most
  visible on a network deployment (`--vps`).

### v0.7.3 — Agents reply on a VPS
- **On a network deployment over plain HTTP, no agent ever replied — total silence.** Reaching the server on a
  LAN/Tailscale IP isn't a browser "secure context", so a browser API used to tag each run was missing; the
  call failed silently and the reply never fired, while the agent kept showing "idle". It only ever worked on
  `localhost`. Affected the Team Room, DMs, Welcome-Home and the Design chat.

### v0.7.2 — Design chat send fix
- **Messages to Grace no longer silently stop sending.** The Design composer's run-lock could stick after an
  SSE reconnect (tab refocus / dropped connection), swallowing every further message; it now clears reliably
  once Grace's reply lands.

### v0.7.1 — Security triage
- **SSRF (Critical) fixed** — the dev-server frameability probe validated the URL with a prefix-only regex that
  let `http://127.0.0.1.evil.com` through; now it parses with `new URL()` and requires an exact loopback host.
- **git argument-injection hardened** on GitHub import (validated branch/repo + `--end-of-options`).
- **Preview address bars are http(s)-only** — a pasted `javascript:`/`data:` URL can't reach the iframe.
- **Defense-in-depth**: `assertOrgId()` before recursive org delete; a length cap on operator regex search.
- Remaining CodeQL path-injection alerts are false positives — the workspace FS jail (`safe()` + `assertOrgId()`,
  lexical + symlink) already contains them; CodeQL just doesn't model it as a sanitizer.
