...
```ts
type CatalogModel = {
  id: string;          // "claude-opus-5", "gpt-6-astra", "grok-4.6"
...
`defaultModelFor()` consulta `DEFAULT_PREFERENCE` (uma ordem de preferência por substring por chave, ex.: `anthropic: ["opus-5", "opus", "sonnet-5", "sonnet"]` — o Opus 5 lidera desde a 0.9.1). A primeira correspondência disponível vence, empates resolvidos pelo lançamento mais recente; sem correspondência de preferência, escolhe o modelo datado mais recente.
...
|---|---|---|---|
| `cli_claude_code` | `claude` | `opus`, `sonnet`, `haiku`, `fable` | ✅ `total_cost_usd` + uso |
| `cli_codex` | `codex` | `gpt-6-astra`, `gpt-5.6-sol`, `gpt-5.6-terra`, `gpt-5.6-luna` | best-effort do JSONL |
...
| `cli_grok` | `grok` | `(default)`, `grok-4.6`, `grok-4.5`, `grok-build-0.1` | ❌ (0) |

> `cli_openclaw` e `cli_hermes` estão **aposentados** (`HIDDEN_CLI_ADAPTERS` em `src/data/model-options.ts`): não aparecem em nenhum picker e não têm entrada em `CLI_MODELS`. As linhas que esta tabela listava para eles estavam obsoletas desde a 0.7.
...
- As entradas de `cli_claude_code` são **aliases**, não ids de modelo — a CLI do Claude resolve cada um para a versão atual (hoje: `fable` → Fable 5.1, `opus` → Opus 5, `sonnet` → Sonnet 5, `haiku` → Haiku 4.5) e rejeita ids completos.
- `CLI_MODELS` é espelhado por `CLI_MODEL_OPTIONS` (`src/data/model-options.ts`) — allowlist do servidor vs. dropdown do cliente. Edite os dois, na mesma ordem: `strongestModelFor` pega a primeira entrada que não seja `(default)`. Fornecedores aposentam slugs, então esta lista apodrece: `gpt-5-codex` (desligado em 2026-07-23) e `grok-code-fast-1` (aposentado em 2026-08-15) ainda estavam listados aqui até a 0.9.2.
