...
| O agente ignora o modelo configurado | O id de modelo falhou na validação do `safeModel` (guarda de injeção) | Use um id de modelo plausível (`opus`/`sonnet`/`haiku`, `gpt-6-astra`, ou `provider/model`). Um valor inválido é descartado e a CLI usa o próprio padrão. |
