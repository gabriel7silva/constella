...
Todos os dez vêm com `provider: cli_claude_code` por padrão. `model` é um alias real da CLI do Claude Code — só `opus`, `sonnet`, `haiku`, `fable` existem para esse adapter (`CLI_MODELS.cli_claude_code` em `src/server/adapters/cli.ts`). Os aliases acompanham o release atual da CLI, então `opus` resolve para o Claude Opus 5.
