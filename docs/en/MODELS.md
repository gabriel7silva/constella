...
`defaultModelFor()` consults `DEFAULT_PREFERENCE` (a substring preference order per key, e.g. `anthropic: ["opus-5", "opus", "sonnet-5", "sonnet"]` — Opus 5 leads since 0.9.1). The first available match wins, ties broken by newest release; with no preference hit it picks the newest dated model.
...
|---|---|---|---|
| `cli_claude_code` | `claude` | `opus`, `sonnet`, `haiku`, `fable` | ✅ `total_cost_usd` + usage |
| `cli_codex` | `codex` | `gpt-6-astra`, `gpt-5.6-sol`, `gpt-5.6-terra`, `gpt-5.6-luna` | best-effort from JSONL |
...
> `cli_openclaw` and `cli_hermes` are **retired** (`HIDDEN_CLI_ADAPTERS` in `src/data/model-options.ts`): they no longer appear in any picker and carry no `CLI_MODELS` entry. The rows this table used to list for them were stale from 0.7.
...
- The `cli_claude_code` entries are **aliases**, not model ids — the Claude CLI resolves each to its current version (today: `fable` → Fable 5.1, `opus` → Opus 5, `sonnet` → Sonnet 5, `haiku` → Haiku 4.5) and rejects full ids.
- `CLI_MODELS` is mirrored by `CLI_MODEL_OPTIONS` (`src/data/model-options.ts`) — server allowlist vs. client dropdown. Edit both, in the same order: `strongestModelFor` takes the first non-`(default)` entry. Vendors retire slugs, so this list rots: `gpt-5-codex` (shut down 2026-07-23) and `grok-code-fast-1` (retired 2026-08-15) were still listed here until 0.9.2.
