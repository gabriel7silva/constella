...
export function proxy(req: NextRequest) {
  ...
  // Worker / agent-hook endpoints — authenticated by CONSTELLA_WORKER_SECRET INSIDE each route (they fail
  // CLOSED with 401 when the header is absent/wrong), never by a session cookie. The worker (bin/worker.mjs)
  // and the agent lock-hook send no session cookie, so without this exemption the session gate below
  // 307-redirects them to /login; the worker's fetch follows it and silently gets the login HTML, which
  // `res.json()` turns into `{}` — that had quietly broken the 24/7 cron tick, the Telegram inbound poll
  // and the disk→DB file-sync (each logged a misleading "200 {}"). Each route still enforces the secret, so
  // exempting them here opens nothing.
  const isWorkerRoute = pathname === "/api/cron/tick" || pathname === "/api/telegram/poll"
    || pathname === "/api/sync/file" || pathname === "/api/locks/acquire";
  if (isAuthRoute || isWorkerRoute) return NextResponse.next();
