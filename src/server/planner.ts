...
export async function groomBacklogFor(orgId: string, workspace: typeof workspaceTable.$inferSelect): Promise<{ ok: boolean; groomed?: number; error?: string }> {
  ...
  let res;
  try {
    res = await runAgentStream(prompt, { orgId, binary, model, timeoutMs: 120_000, token: uid() }, () => {});
  } finally {
 ...
 */
export async function generatePlan(opts?: { brief?: string; goalTitle?: string; channel?: string; requestText?: string }): Promise<{ ok: boolean; started?: boolean; error?: string }> {
  const { org, workspace } = await requireWorkspace();
 ...
 */
export async function startNewWork(input: { title?: string; brief: string }): Promise<{ ok: boolean; started?: boolean; error?: string }> {
 ...
 */
export async function planFromConversation(channel: string): Promise<{ ok: boolean; started?: boolean; error?: string }> {
  const { workspace } = await requireWorkspace();
  ...
  // (design-gate visual/backend check), kept separate from the full-conversation `brief` above.
  const requestText = [...recent].reverse().find((m) => m.fromKind === "operator")?.text ?? "";
