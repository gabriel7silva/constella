...
export async function detectProviders(): Promise<{ detected: DetectedProvider[]; cloud: { id: string; name: string; adapter: string }[] }> {
  ...
  const [claude, codex, aider, opencode, copilot, cursor, cline, kilo, grok, oll] = await Promise.all([
    cliVersion("claude"), cliVersion("codex"),
    cliVersion("aider"), cliVersion("opencode"), cliVersion("copilot"), cliVersion("cursor-agent"), cliVersion("cline"), cliVersion("kilocode"), cliVersion("grok"),
    ollamaInfo(),
  ]);
  ...
  if (grok) detected.push({ id: "grok_build_cli", name: "Grok Build (xAI)", kind: "cli", adapter: "cli_grok", models: CLI_MODELS.cli_grok });
