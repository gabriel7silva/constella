...
export async function grepWorkspace(query: string, opts?: { caseSensitive?: boolean; wholeWord?: boolean; regex?: boolean }):
  ...
  // A raw operator-supplied regex runs on the single-threaded server; cap its length to bound catastrophic
  // backtracking (ReDoS). Plain-text search (escaped) is unaffected by this.
  if (opts?.regex && q.length > 1000) return { ok: false, files: [], truncated: false, scanned: 0, error: "Regular expression is too long." };
