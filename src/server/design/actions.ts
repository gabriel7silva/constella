...
/** The minimal design context the moved Design chat (design-chat.tsx, now in Grace's DM) needs for its
 *  suggestion chips: whether prototype screens exist + whether the design is approved. */
export async function getDesignChatContext(): Promise<{ approved: boolean; designMockFiles: string[] }> {
  const { org, workspace } = await requireWorkspace();
  ensureDesignTables();
  const ctx = gatherDesignContext(org.id, workspace);
  return { approved: !!ctx.approved, designMockFiles: ctx.designMockFiles };
}
...
• When they DO ask for a concrete screen, component or visual change: consult your design skills, then BUILD IT LIVE as described above (skeleton file first, then incremental edits under design-mock/screens/), TEST what you built (does it render? do the requested changes appear?), and reply with a 2-3 sentence summary of what you built, what you validated, and what's next.`,
      ...
      const res = await runAgentStream(fullPrompt, { orgId: org.id, binary, model, timeoutMs: 1_200_000, agentId: grace.id, agentHandle: grace.handle, token: runId },
      ...
      // Deliver the REAL reply whenever Grace produced text — even when the run ended non-clean (ok=false via
      // an early stop / turn-cap still yields her real final summary). Only the genuine no-text case gets the
      // "(couldn't respond …)" template. (Bug: a long build showed "(couldn't respond: <her actual reply>)".)
      const reply = res.cancelled ? "Stopped by operator." : (res.text?.trim() || (res.ok ? "Prototype updated." : `(couldn't respond: ${friendlyAgentError(res.error)})`));
      await db.insert(message).values({
...
async function runGraceHandoff(orgId: string, workspace: WorkspaceRow, work?: { brief: string; goalTitle?: string }): Promise<void> {
  ...
  void (async () => {
    ...
    try {
      ...
      const touched = new Set<string>();
      const res = await runAgentStream(prompt, { orgId, binary, model, timeoutMs: 1_200_000, agentId: grace.id, agentHandle: grace.handle, token: runId },
        (ev) => {
      ...
      const reply = res.cancelled ? "Stopped by operator." : (res.text?.trim() || (res.ok ? "Handoff documentation ready — sending to the CEO." : `(couldn't finish the docs: ${friendlyAgentError(res.error)})`));
