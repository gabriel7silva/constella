...
export async function askDesign(prompt: string, attachments?: { name: string; type: string; size: number; path: string }[], selection?: CanvasSelection): Promise<{ ok: boolean; started?: boolean; error?: string }> {
  ...
  // runId collapse into the persisted bubble (the ChatStream contract used across every channel).
  const runId = uid();
...
• When they DO ask for a concrete screen, component or visual change: consult your design skills, then BUILD IT LIVE as described above (skeleton file first, then incremental edits under design-mock/screens/), TEST what you built (does it render? do the requested changes appear?), and reply with a 2-3 sentence summary of what you built, what you validated, and what's next.`,
      ...
      const res = await runAgentStream(fullPrompt, { orgId: org.id, binary, model, timeoutMs: 600_000, agentId: grace.id, agentHandle: grace.handle },
      ...
      const reply = res.ok ? (res.text?.trim() || "Prototype updated.") : `(couldn't respond: ${friendlyAgentError(res.error || res.text)})`;
      await db.insert(message).values({
...
async function runGraceHandoff(orgId: string, workspace: WorkspaceRow, work?: { brief: string; goalTitle?: string }): Promise<void> {
  ...
  void (async () => {
    ...
    try {
      ...
      const touched = new Set<string>();
      const res = await runAgentStream(prompt, { orgId, binary, model, timeoutMs: 600_000, agentId: grace.id, agentHandle: grace.handle },
        (ev) => {
      ...
      const reply = res.ok ? (res.text?.trim() || "Handoff documentation ready — sending to the CEO.") : `(couldn't finish the docs: ${friendlyAgentError(res.error || res.text)})`;
