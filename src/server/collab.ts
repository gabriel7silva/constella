 ...
 * `token` (client-generated for an operator-triggered chat turn) registers the run so cancelRun
 * can interrupt it mid-flight; omitted (undefined) for agent-triggered hand-offs.
 */
export async function replyInChannel(orgId: string, ws: Ws, channel: string, a: Ag, mode: "chat" | "work", handoff?: { from: string; text: string }, token?: string): Promise<{ text: string; planRequested: boolean }> {
  const msgId = token || uid();
  ...
  const res = await runAgentRuntime(prompt, rt, { orgId, model: aliasModel(binary, a.model), timeoutMs: 180_000, effort: a.effort, token },
  ...
  await db.insert(message).values({
    ...
    text: res.cancelled ? "Stopped by operator." : res.ok && shown ? shown.slice(0, 4000) : `(${a.name} couldn't respond: ${friendlyAgentError(res.error)})`,
  ...
  return { text: res.cancelled ? "Stopped by operator." : res.ok ? clean : "", planRequested };
}
