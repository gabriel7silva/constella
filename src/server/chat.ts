 ...
 *  `token` (client-generated, e.g. crypto.randomUUID()) registers the run so cancelRun can stop it mid-flight. */
export async function agentRespond(channel: string, handle: string, token?: string): Promise<void> {
  const { org, workspace } = await requireWorkspace();
  ...
  try {
    const r = await replyInChannel(org.id, workspace, channel, a, "chat", undefined, token);
    reply = r.text;
  ...
  if (planRequested) {
    try {
      ...
      // `started` distinguishes "a plan job is actually running" from "the call resolved but did
      // nothing" (already drafting, or a design-gate/parse/failure hold — those now post their OWN
      // honest outcome message from inside the job itself, so don't repeat/contradict it here).
      const detail = r.started === false
        ? `I'm already drafting a plan — hang tight, I'll post it here when it's ready.`
        : !r.ok
...
export async function clearConversation(channel: string): Promise<{ ok: boolean }> {
  const { workspace } = await requireWorkspace();
  if (!channel.startsWith("dm:") && channel !== "telegram" && channel !== "room" && channel !== "design") return { ok: false };
  if (channel.startsWith("dm:")) {
