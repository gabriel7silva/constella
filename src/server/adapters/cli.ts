...
import { orgRoot, constellaHome, assertOrgId, readContained, writeContained } from "@/lib/fs-workspace";
...
export async function runGrok(prompt: string, opts: { model?: string; orgId: string; timeoutMs?: number; token?: string }): Promise<CliResult> {
  // Assert the org id HERE, not only inside orgRoot(), so the charset gate is local to the function that
  // builds the spawn cwd — the value flows straight into a filesystem path and a child process.
  assertOrgId(opts.orgId);
  const cwd = orgRoot(opts.orgId);
  mkdirSync(cwd, { recursive: true });
  const start = Date.now();
  return new Promise<CliResult>((done) => {
    ...
    function onMessage(m: Record<string, unknown>) {
      ...
      if (method && mid != null) { // a REQUEST FROM the agent — must answer or it hangs
        // ACP fs paths are ABSOLUTE and chosen by the model — the least trusted input we handle. Both go
        // through the workspace jail in fs-workspace (`safe()`: lexical containment + a realpath ancestor
        // check), which also blocks the symlink-tunnel escape the old local lexical-only guard let through.
        if (method === "fs/read_text_file") {
          const r = readContained(cwd, params.path);
          if (r.ok) return write({ jsonrpc: "2.0", id: mid, result: { content: r.content } });
          return write({ jsonrpc: "2.0", id: mid, error: r.reason === "outside" ? { code: -32602, message: "path outside workspace" } : { code: -32000, message: "read failed" } });
        }
        if (method === "fs/write_text_file") {
          const w = writeContained(cwd, params.path, String(params.content ?? ""));
          if (w.ok) return write({ jsonrpc: "2.0", id: mid, result: {} });
          return write({ jsonrpc: "2.0", id: mid, error: w.reason === "outside" ? { code: -32602, message: "path outside workspace" } : { code: -32000, message: "write failed" } });
        }
...
export async function detectCliAuth(binary: CliBinary): Promise<AuthState> {
  // Read the env var first instead of calling homedir() directly — same value (this is the order
  // os.homedir() itself uses), but NOT a compile-time constant. Next's file tracer (@vercel/nft)
  // statically evaluates `homedir()`, and joining that known base with a RUNTIME name below makes the
  // tracer give up and trace `<home>/**/*` — i.e. `next build` globs the operator's ENTIRE home
  // directory. That is slow everywhere and fails the build outright on Windows, where the legacy
  // "Meus Documentos" / "My Documents" junction is deny-listed and scandir returns EPERM.
 ...
 *
 *  MIRRORED in `CLI_MODEL_OPTIONS` (`@/data/model-options`) — that one is client-safe and feeds the
 *  dropdowns, this one is the server-side allowlist. Edit BOTH, in the SAME ORDER: `strongestModelFor`
 *  returns the first non-`(default)` entry here, so ordering picks the reviewer's model.
 *
 *  Refreshed 2026-09-07. Vendors RETIRE slugs, so this list rots: `gpt-5-codex` was shut down 2026-07-23
 *  and `grok-code-fast-1` on 2026-08-15 — both were still offered here until 0.9.2. */
export const CLI_MODELS: Record<string, string[]> = {
  // Aliases, not ids — the Claude CLI resolves each to its current version (fable → Fable 5.1, opus → Opus 5,
  // sonnet → Sonnet 5, haiku → Haiku 4.5). It REJECTS full model ids, so this list stays alias-only.
  cli_claude_code: ["opus", "sonnet", "haiku", "fable"],
  cli_codex: ["gpt-6-astra", "gpt-5.6-sol", "gpt-5.6-terra", "gpt-5.6-luna"],
  cli_aider: ["(default)", "anthropic/claude-sonnet-5", "openai/gpt-5.6-sol", "deepseek/deepseek-chat"],
  cli_opencode: ["(default)", "anthropic/claude-sonnet-5", "openai/gpt-5.6-sol"],
  // copilot/cursor: ids come from the vendor's own model menu, not from us — left as the vendor spells them.
  cli_copilot: ["(default)", "claude-sonnet-4.5", "gpt-5"],
  cli_cursor: ["(default)", "claude-4.5-sonnet", "gpt-5"],
  cli_cline: ["(default)"],
  cli_kilo: ["(default)"],
  cli_grok: ["(default)", "grok-4.6", "grok-4.5", "grok-build-0.1"],
};
