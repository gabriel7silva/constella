...
/** Grok Build — xAI's terminal-native coding agent (Grok 4.5). Headless: `grok -p <p> [-m model]`. Auth via the
 *  `grok` browser sign-in (SuperGrok / X Premium+) or the GROK_CODE_XAI_API_KEY env. Agentic (edits files + runs
 *  shell). Uses a BARE model id (`-m grok-4.5`), not a provider/model slash. No token/cost emitted → 0 (honest). */
export async function runGrok(prompt: string, opts: { model?: string; orgId: string; timeoutMs?: number; token?: string }): Promise<CliResult> {
  ...
  const args = ["-p", prompt];
  if (model) args.push("-m", model);
  return runGenericCli("grok", args, opts);
}
