...
export type ProcResult = { code: number | null; stdout: string; stderr: string; durationMs: number; timedOut: boolean };

// Running agent processes keyed by an abort token (= taskId). Lets cancelGoal SIGKILL the
// in-flight CLI run for a cancelled goal mid-flight, not just gate the next one.
...
export function abortRun(token: string): boolean {
  ...
  if (!child) return false; // not spawned yet → the ABORTED mark will kill it on registration
  ACTIVE.delete(token);
...
function registerChild(token: string | undefined, child: ReturnType<typeof spawn>): void {
  if (!token) return;
  if (ABORTED.has(token)) { ABORTED.delete(token); try { child.kill("SIGKILL"); } catch {} return; }
  ACTIVE.set(token, child);
}
function unregisterChild(token?: string): void { if (token) { ACTIVE.delete(token); ABORTED.delete(token); } }
...
function runProc(cmd: string, args: string[], input: string, cwd: string, timeoutMs: number, env?: NodeJS.ProcessEnv, token?: string, shell: boolean = IS_WIN): Promise<ProcResult> {
  return new Promise((resolve) => {
    ...
    const child = spawn(cmd, args, { cwd, shell, windowsHide: true, env: env ?? process.env });
    registerChild(token, child);
    let stdout = "", stderr = "", timedOut = false;
    const fin = () => unregisterChild(token);
    const timer = setTimeout(() => { timedOut = true; try { child.kill("SIGKILL"); } catch {} }, timeoutMs);
    child.stdout.on("data", (d) => (stdout += d.toString()));
    child.stderr.on("data", (d) => (stderr += d.toString()));
    child.on("error", (e) => { clearTimeout(timer); fin(); resolve({ code: -1, stdout, stderr: stderr + String(e), durationMs: Date.now() - start, timedOut }); });
    child.on("close", (code) => { clearTimeout(timer); fin(); resolve({ code, stdout, stderr, durationMs: Date.now() - start, timedOut }); });
...
function base(binary: CliBinary, model: string | undefined, durationMs: number, error: string): CliResult {
  return { ok: false, text: "", usd: 0, inputTokens: 0, outputTokens: 0, durationMs, binary, model, error };
}
 ...
 *  versions → shorter default timeout (the runner's SIGKILL still applies). Auth via `cursor-agent login`. */
...
export async function runClaudeStream(prompt: string, opts: { model?: string; orgId: string; timeoutMs?: number; token?: string; agentId?: string; agentHandle?: string }, onEvent: (e: StreamEvent) => void): Promise<CliResult> {
  ...
  return new Promise((resolve) => {
    const start = Date.now();
    const child = spawn("claude", args, { cwd, shell: IS_WIN, windowsHide: true, env: claudeEnv(opts, agentDir) });
    registerChild(opts.token, child);
    let buf = "", text = "", usd = 0, inTok = 0, outTok = 0, ok = false, error: string | undefined;
    ...
    const timer = setTimeout(() => { try { child.kill("SIGKILL"); } catch {} }, opts.timeoutMs ?? 240_000);
    ...
    const fin = () => unregisterChild(opts.token);
    child.on("error", (e) => { clearTimeout(timer); fin(); resolve(base("claude", opts.model, Date.now() - start, String(e))); });
    child.on("close", () => { clearTimeout(timer); fin(); onEvent({ kind: "done" }); resolve({ ok, text, usd, inputTokens: inTok, outputTokens: outTok, durationMs: Date.now() - start, binary: "claude", model: opts.model, error }); });
 ...
 *  timeout / SIGKILL — those are long tasks or operator cancellations and must never auto-retry. */
 ...
 *  error or a success returns immediately; cancellations (SIGKILL) don't match isRetryableAgentError → no retry. */
export async function runAgentStream(prompt: string, opts: { orgId: string; binary?: CliBinary; model?: string; timeoutMs?: number; token?: string; agentId?: string; agentHandle?: string; effort?: Effort }, onEvent: (e: StreamEvent) => void): Promise<CliResult> {
  ...
  for (let attempt = 0; attempt <= AGENT_RETRY_BACKOFF_MS.length; attempt++) {
    ...
    if (r.ok || !isRetryableAgentError(r.error)) return r;
    last = r;
 ...
 *  `token` (= taskId) registers the child so cancelGoal can SIGKILL it mid-flight. */
