...
export async function runClaudeStream(prompt: string, opts: { model?: string; orgId: string; timeoutMs?: number; token?: string; agentId?: string; agentHandle?: string }, onEvent: (e: StreamEvent) => void): Promise<CliResult> {
  ...
  return new Promise((resolve) => {
    ...
    child.on("close", () => {
      ...
      fin();
      onEvent({ kind: "done" });
      resolve({ ok, text, usd, inputTokens: inTok, outputTokens: outTok, durationMs: Date.now() - start, binary: "claude", model: opts.model, error: cancelled ? "stopped by operator" : error, cancelled });
    });
    child.stdin.on("error", () => {});
