...
import { constellaHome, assertOrgId } from "@/lib/fs-workspace";
...
export async function deleteOrg(orgId: string) {
  ...
  if (!s || !(await owns(s.user.id, orgId))) return;
  try {
    assertOrgId(orgId); // belt-and-suspenders before a recursive delete (owns() already gates on DB membership)
