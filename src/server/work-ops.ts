...
/** Park a goal's tasks out of the runnable set (+ interrupt any in-flight run, mirror issues,
...
export async function parkGoalTasks(wsId: string, goalId: string): Promise<void> {
  ...
  for (const t of tasks) {
    abortRun(t.id); // interrupt the live CLI process (or mark so a mid-claim run stops on spawn)
