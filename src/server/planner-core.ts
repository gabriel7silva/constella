...
export async function planFromConversationFor(orgId: string, ws: typeof workspaceTable.$inferSelect, channel: string): Promise<{ ok: boolean; started?: boolean; error?: string }> {
  ...
  if (!msgs.length) return { ok: false, error: "No conversation to plan from yet." };
  // The operator's own LATEST message, undiluted by any agent reply — used to classify THIS request
  // (design-gate visual/backend check), kept separate from `brief` (the full back-and-forth) below.
  const requestText = msgs.find((m) => m.fromKind === "operator")?.text ?? "";
...
export async function generatePlanFor(orgId: string, workspace: typeof workspaceTable.$inferSelect, opts?: { brief?: string; goalTitle?: string; channel?: string; requestText?: string }): Promise<{ ok: boolean; started?: boolean; error?: string }> {
...
async function runPlanJob(ctx: { orgId: string; workspace: typeof workspaceTable.$inferSelect; ada: AdaRow; agents: AdaRow[]; runId: string; opts?: { brief?: string; goalTitle?: string; channel?: string; requestText?: string } }): Promise<void> {
 ...
 try {
  ...
  // context, so Grace's foundation scaffold is triggered from the Design module on open — not here.
  {
    ...
    const autoSkipDesign = !!wsSettings.design?.autoSkip;
    ...
    // The THIS-REQUEST signal, undiluted by mission/stack/prior agent replies (see `requestText` above) — a
    // backend-only ask (e.g. "configure the dev server") must never route through Grace, even on a project
    // that's never been designed yet and even if the wider mission/stack blob reads visual.
    const requestText = (opts?.requestText || opts?.brief || "").trim();
    const requestIsBackendOnly = isNewWork && !!requestText && looksBackendOnly(requestText);
    ...
    const needsDesign = !requestIsBackendOnly && (designNeedsPrototype(dctx) || (isNewWork && looksVisual(brief)));
    if (isVisualProduct && !gate.skip && !autoSkipDesign && needsDesign) {
      ...
      // Chat-originated requests silently held here previously — the operator watching a DM/room saw nothing
      // after Ada's earlier "I'm drafting…" confirmation. Post the real outcome into the channel they're on.
      await db.insert(message).values({
        id: uid(), workspaceId: workspace.id, channel: opts?.channel ?? "room", fromKind: "agent", fromHandle: ada.handle,
        text: `This looks like it needs a quick design pass first — I've sent it to Grace in the Design module. Open Design, prototype & approve the UI (or skip design there to go straight to the plan), and I'll pick this back up.`,
        createdAt: new Date(),
      });
  ...
  const res = await runAgentStream(prompt, { orgId: org.id, binary, model, timeoutMs: 300_000, token: runId },
    (ev) => { void emit(workspace.id, { runId, channel: "planner", agentId: ada.id, kind: ev.kind, target: ev.target, detail: ev.detail }); });

  if (res.cancelled) {
    await emit(workspace.id, { runId, channel: "planner", agentId: ada.id, kind: "error", target: "Stopped by operator." });
    await db.insert(message).values({ id: uid(), workspaceId: workspace.id, channel: opts?.channel ?? "room", fromKind: "agent", fromHandle: ada.handle, text: "Stopped by operator.", createdAt: new Date() });
    return;
  }
  ...
  if (!parsed || !parsed.specs?.length) {
    ...
    await db.insert(message).values({ id: uid(), workspaceId: workspace.id, channel: opts?.channel ?? "room", fromKind: "agent", fromHandle: ada.handle, text: `I couldn't turn that into a structured plan — try rephrasing with a bit more detail and I'll take another pass.`, createdAt: new Date() });
    return;
  }
  ...
  // CEO narrates the plan — into the channel the request came from (a DM stays a DM), falling back to
  // the team room for the standing/first-plan case (no originating chat channel).
  await db.insert(message).values({
    id: uid(), workspaceId: workspace.id, channel: opts?.channel ?? "room", fromKind: "agent", fromHandle: ada.handle,
  ...
  try { await db.insert(message).values({ id: uid(), workspaceId: workspace.id, channel: opts?.channel ?? "room", fromKind: "agent", fromHandle: ada.handle, text: `Something went wrong while drafting the plan — try again in a moment.`, createdAt: new Date() }); } catch { /* ignore */ }
 } finally {
