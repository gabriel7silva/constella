 ...
 *  `token` (client-generated) registers the run so cancelRun/Stop-All can interrupt it mid-flight:
 *  CLI runs are process-killed, HTTP/local streams are aborted through AbortController. */
export async function runAgentRuntime(
  prompt: string,
  rt: Runtime,
  opts: { orgId: string; model?: string; timeoutMs?: number; effort?: Effort; token?: string },
  onEvent: (e: StreamEvent) => void,
): Promise<CliResult> {
  if (rt.kind === "http") return runHttpStream(prompt, rt.http, { timeoutMs: opts.timeoutMs, token: opts.token }, onEvent);
  return runAgentStream(prompt, { orgId: opts.orgId, binary: rt.binary, model: opts.model, timeoutMs: opts.timeoutMs, effort: opts.effort, token: opts.token }, onEvent);
}
