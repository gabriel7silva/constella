...
import { join, normalize, sep, dirname, isAbsolute, relative } from "node:path";
...
/* ------------------------------------------------------------------ containment for ABSOLUTE paths */
/**
 * The wrappers above take a workspace-relative path, which is the shape our own code uses. An
 * *agent protocol* hands us something else: ACP (`fs/read_text_file`, `fs/write_text_file`) sends
 * ABSOLUTE paths chosen by the model, i.e. the least trusted input in the system — a prompt-injected
 * agent will happily ask for `C:\Users\...\.ssh\id_rsa` or plant a symlink and read through it.
 *
 * These three run such a path through the SAME `safe()` jail (lexical containment + realpath ancestor
 * check), returning null/false instead of throwing so a protocol handler can answer with a JSON-RPC
 * error. The fs calls live HERE, behind the guard, rather than at the call site.
 */
export function containedPath(root: string, p: unknown): string | null {
  if (typeof p !== "string" || !p) return null;
  // Two Windows shapes that neither isAbsolute() nor join() handles the way you'd hope, so they are
  // refused outright rather than guessed at:
  //  - DRIVE-RELATIVE ("C:notes.txt") — isAbsolute() says false, so it would be joined under root as a
  //    literal segment containing a colon (NTFS alternate-data-stream syntax). Never legitimate here.
  //  - UNC / device ("\\\\server\\share", "\\\\?\\C:\\...") — resolves off-machine or bypasses normalization.
  if (/^[A-Za-z]:(?![\\/])/.test(p) || /^[\\/]{2}/.test(p)) return null;
  let rel = p;
  if (isAbsolute(p)) {
    rel = relative(root, p);
    // Reject before handing it to safe(): `relative()` returns a `../…` path for a sibling directory, but
    // for a path on ANOTHER DRIVE (root on C:, "/etc/passwd" resolving to L:\etc\passwd) it returns an
    // ABSOLUTE path instead. join() would then bury that under root as a literal `L:\etc\passwd` segment
    // and the containment check would pass — contained, but on a nonsense path we should never build.
    if (isAbsolute(rel) || rel === ".." || rel.startsWith(".." + sep)) return null;
  }
  // safe() re-checks lexically and then against the realpath of the nearest existing ancestor, which is
  // what stops a symlink planted inside the workspace from tunnelling out.
  try { return safe(root, rel); } catch { return null; }
}

/** Why a contained read/write didn't happen. Callers map these to distinct protocol errors —
 *  `outside` is the security refusal, `failed` is ordinary I/O (missing file, permissions). */
export type ContainedFail = { ok: false; reason: "outside" | "failed" };
export type ContainedRead = { ok: true; content: string } | ContainedFail;

/** Read a file at an absolute-or-relative path contained in `root`. */
export function readContained(root: string, p: unknown): ContainedRead {
  const abs = containedPath(root, p);
  if (!abs) return { ok: false, reason: "outside" };
  try { return { ok: true, content: readFileSync(abs, "utf8") }; } catch { return { ok: false, reason: "failed" }; }
}

/** Write a file at an absolute-or-relative path contained in `root`, creating parent dirs. */
export function writeContained(root: string, p: unknown, content: string): { ok: true } | ContainedFail {
  const abs = containedPath(root, p);
  if (!abs) return { ok: false, reason: "outside" };
  try { mkdirSync(dirname(abs), { recursive: true }); writeFileSync(abs, content, "utf8"); return { ok: true }; } catch { return { ok: false, reason: "failed" }; }
}
