...
export async function POST(req: Request) {
  ...
  if (typeof orgId !== "string" || typeof path !== "string" || !orgId || !path) {
    return NextResponse.json({ ok: false, error: "missing orgId/path" }, { status: 400 });
  }
  // `orgId` comes straight off the JSON body and reaches orgRoot() → a filesystem path. orgRoot() asserts
  // the charset itself, but it THROWS — do the same check here so a malformed body is a clean 400, not a 500.
