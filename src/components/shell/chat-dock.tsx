...
import { askDesign } from "@/server/design/actions";
...
export function ChatDock({ agents, operator }: { agents: Agent[]; operator?: { name: string; image?: string | null } }) {
  ...
  const graceSelRef = useRef<CanvasSelection | null>(null); // a canvas element the operator picked → attached to the next design send
  ...
  useEffect(() => {
    function openGrace(e: Event) {
      ...
      graceSelRef.current = d?.selection ?? null;
      if (d?.text) { setSeed(d.text); setSeedKey((k) => k + 1); }
    }
  ...
  function doSend(text: string, attachments?: Attachment[]) {
    ...
    setSendBusy(true);
    start(async () => {
      // Grace's DM rides the "design" channel → build/update the canvas via askDesign (it persists BOTH the
      // operator turn and Grace's reply and streams to the design channel; the SSE tail renders them here).
      if (channel === "design") {
        const h = designHandle ?? "grace";
        const selection = graceSelRef.current ?? undefined;
        graceSelRef.current = null;
        const token = newRunToken();
        runTokens.current = { ...runTokens.current, [h]: token };
        setTyping([h]);
        try {
          if (stoppedSends.current.has(seq)) { void cancelRunClient(token); }
          else await askDesign(text, attachments, selection, token);
        } catch (e) { console.error("[design-dm] send/build failed:", e); }
        finally {
