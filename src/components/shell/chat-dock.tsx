...
export function ChatDock({ agents, operator }: { agents: Agent[]; operator?: { name: string; image?: string | null } }) {
  ...
  useEffect(() => {
    ...
    function connect() {
      ...
      es.addEventListener("msg", (e) => {
        ...
        if (row.kind === "cleared") {
          setMsgs([row]); setEvents([]); setTyping([]);
  ...
  function doSend(text: string, attachments?: Attachment[]) {
    start(async () => {
      ...
      if (responders.length) {
        ...
        for (const h of responders) { await agentRespond(channel, h); setTyping((cur) => cur.filter((x) => x !== h)); }
        setTyping([]);
      }
      if (inDm) await loadPreviews();
    });
  ...
  const liveRuns = Object.entries(byRun).filter(([rid]) => !msgIds.has(rid));
    ...
    <div className={"chat-dock " + side} style={{ width }}>
      ...
      {inDm && dmAgent ? (
        <>
          ...
          <ChatComposer key={"dm:" + dmH + ":" + seedKey} onSend={doSend} agents={agents} defaultText={seed} placeholder={t("chat.dock.messageAgent", { name: dmAgent.name })} />
        </>
      ) : inPeople ? (
        ...
        <>
          ...
          {tg && !tg.connected ? (
            ...
            <>
              ...
              <ChatComposer key={"telegram:" + seedKey} onSend={doSend} agents={agents} placeholder={t("chat.tg.composerPlaceholder")} />
            </>
          )}
        </>
      ) : (
        <>
          ...
          <ChatComposer key={"room:" + seedKey} onSend={doSend} agents={agents} defaultText={view === "room" ? seed : ""} requireMention placeholder={t("chat.dock.roomPlaceholder")} refs={refs} />
        </>
      )}
      {confirmDlg && (
