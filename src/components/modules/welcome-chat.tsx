...
export function WelcomeChat({ agents, operator }: { agents: Agent[]; operator: { name: string; image?: string | null } }) {
  ...
  return (
    <section className={"welcome-chat" + (full ? " full" : "")}>
      ...
      {!inPeople && !isDesignDm && !(inTg && tg && !tg.connected) && (
      ...
      {inDm && dmAgent ? (
        <>
          ...
          {isDesignDm ? (
            <DesignChat grace={{ id: dmAgent.id, name: dmAgent.name, handle: dmAgent.handle, color: dmAgent.color, image: dmAgent.image }} operator={operator} />
          ) : (
            <>
