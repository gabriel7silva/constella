...
export function WelcomeChat({ agents, operator }: { agents: Agent[]; operator: { name: string; image?: string | null } }) {
  ...
  useEffect(() => {
    ...
    function connect() {
      ...
      es.addEventListener("msg", (e) => {
        ...
        if (row.kind === "cleared") { setMsgs([row]); setEvents([]); setTyping([]); return; }
  ...
  function doClear() {
    ...
    start(async () => { await clearConversation(channel).catch(() => {}); setMsgs([]); setEvents([]); setTyping([]); await loadCtx(); });
  }
    ...
    <section className={"welcome-chat" + (full ? " full" : "")}>
      ...
      {inDm && dmAgent ? (
        <>
          ...
          <ChatComposer onSend={doSend} agents={agents} requireMention={false} placeholder={composerPlaceholder} />
        </>
      ) : inPeople ? (
