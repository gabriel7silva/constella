...
export function WelcomeChat({ agents, operator }: { agents: Agent[]; operator: { name: string; image?: string | null } }) {
  ...
  // Grace's DM IS the Design chat — it rides the "design" channel and builds the canvas via askDesign.
  ...
  function doSend(text: string, attachments?: Attachment[]) {
    ...
    setSendBusy(true);
    start(async () => {
      // Grace's DM rides the "design" channel → build the canvas via askDesign (persists both turns + streams).
      if (channel === "design") {
        ...
        try {
          ...
          else await askDesign(text, attachments, undefined, token);
        } catch (e) { console.error("[welcome-chat] design send failed:", e); }
        finally {
      ...
      } catch (e) { console.error("[welcome-chat] send/reply failed:", e); /* the SSE stream surfaces server-side failures; log client-side ones so they're never silent */ }
      finally {
