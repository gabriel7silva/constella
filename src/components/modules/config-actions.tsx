...
import { updateWorkspace, setMonthlyCap, setEditorSettings, toggleIntegration, setAgentRuntime, setDesignSettings } from "@/server/actions/config-actions";
...
/** Design Gate default — real, persisted to workspace.settings.design. Off = always wait for Grace to
 *  prototype/approve a visual request first (today's default); on = new visual work always skips the
 *  gate and goes straight to the plan (the operator can still open Design manually any time). */
export function DesignGateSettings({ design }: { design?: { autoSkip?: boolean } }) {
  const t = useT();
  const [autoSkip, setAutoSkip] = useState(!!design?.autoSkip);
  const [, start] = useTransition();
  const toggle = () => { const on = !autoSkip; setAutoSkip(on); start(() => setDesignSettings({ autoSkip: on })); };
  return (
    <div className="set-row" style={{ paddingTop: 0 }}>
      <div className="sr-main"><div className="sr-title">{t("config.design.autoSkip")}</div><div className="sr-sub">{t("config.design.autoSkipDesc")}</div></div>
      <div className={"toggle" + (autoSkip ? " on" : "")} onClick={toggle} />
    </div>
  );
}
