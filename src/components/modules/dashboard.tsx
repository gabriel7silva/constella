...
/** Relative-time text as of `now` (a caller-supplied instant, never read fresh here) — `now === 0` is the
 *  pre-mount sentinel and always renders "—", same on the server and the client's first pass (no diff).
 *  Reading Date.now() directly during render (the previous `ago()`) mismatches the server's render instant
 *  against the client's hydration instant ("0s" vs "1s") — a real hydration error. One shared `now` (from
 *  useNow() below) also keeps every relative-time string in a render internally consistent. */
function ago(now: number, ms: number | null): string {
  if (!ms || !now) return "—";
...
/** The "now" every `ago()` call in this component renders against — 0 until mounted, then ticks. */
    ...
    <div className="dash">
      ...
      <div className="dash-filters">
        ...
        <div className="df-meta">
          <span className="df-updated">{t("dash.updated")} {ago(now, data.generatedAt)}</span>
      ...
      <div className="dash-grid">
        ...
        <div className="dash-card stat-card" style={{ gridColumn: "span 3" }}>
          ...
          <div className="sc-foot"><Icon name="pulse" size={11} /> {t("dash.ag.heartbeat")} {ago(now, c.agents.lastHeartbeat)}</div>
        </div>
        ...
        <div className="dash-card stat-card" style={{ gridColumn: "span 3" }}>
          ...
          <div className="sc-foot"><Icon name="refresh" size={11} /> {t("dash.lastScan")} {ago(now, c.security.lastScan)}</div>
        </div>
          ...
          {data.execution && data.execution.runState === "running" ? (
            <div className="exec-panel">
              <div className="exec-head">
                ...
                <span className="exec-elapsed mono">{data.execution.elapsedMs && now ? ago(now, now - data.execution.elapsedMs) : ""}</span>
              </div>
                ...
                return (
                  <div className="agent-card" key={a.id}>
                    ...
                    <div className="ac-main">
                      ...
                      <div className="ac-hb"><Icon name="pulse" size={10} /> {ago(now, a.lastPulseMs)}</div>
                    </div>
                  </div>
                );
              })}
        ...
        <div className="dash-card" style={{ gridColumn: "span 5" }}>
          ...
          {data.alerts.length === 0 ? (
            ...
            <Link href="/inbox" key={al.key} className="alert-group">
              ...
              <span className="ag-time">{ago(now, al.latestAtMs)}</span>
            </Link>
          ))}
        </div>
      </div>
            ...
            <div className="kb-body">
              ...
              <div className="kb-right">
                <div className="kb-tiles">
                  ...
                  <div className="ks"><div className="ks-v">{ago(now, data.kb.lastReindexMs)}</div><div className="ks-k">{t("dash.kb.lastReindex")}</div></div>
