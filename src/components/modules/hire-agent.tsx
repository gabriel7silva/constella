              ...
              <div className="set-grid">
                ...
                <div className="form-field"><label className="form-label">{t("agent.effort.label")}</label>
                  <div style={{ display: "flex", gap: 6 }}>
                    {EFFORTS.map((e) => (
                      <button key={e} type="button" className="chip-sm" style={{ flex: 1, justifyContent: "center", ...(effort === e ? { background: "var(--accent)", color: "var(--accent-fg)" } : {}) }} onClick={() => setEffort(e)}>{t("agent.effort." + e)}</button>
                    ))}
                  </div>
                </div>
              </div>
