...
function EffortControl({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  const t = useT();
  return (
    <div className="temp-wrap">
      ...
      <div style={{ display: "flex", gap: 6, marginTop: 2 }}>
        {EFFORTS.map((e) => (
          <button key={e} type="button" className="chip-sm" style={{ flex: 1, justifyContent: "center", ...(value === e ? { background: "var(--accent)", color: "var(--accent-fg)" } : {}) }} onClick={() => onChange(e)}>{t("agent.effort." + e)}</button>
        ))}
      </div>
