...
### Fixed
- **The internal release guide was shipping inside the public npm tarball.** `scripts/publish-public.mjs`
  has always excluded `docs/en/PUBLISHING.md` / `docs/pt/PUBLISHING.md` from the public **Git** mirror — it is
  the internal guide and it names the private dev remote — but the **npm** channel never carried the matching
  exclusion. `files[]` lists `docs`, and that is an allowlist entry, so npm packed the whole tree. Every
  published tarball up to and including 0.9.2 carried both files (verified with `npm pack constellai@0.9.2`);
  no credentials were in them, so there is nothing to rotate — the two channels simply have to agree. Two
  `files[]` negations fix it, and the tarball drops from 2001 files to 1999. The negations must stay directly
  below `docs`: a `files` negation only applies to an include that precedes it.

## [0.9.2] — "Every security alert closed + the 2026-09 model lineup"

### Security
- **Patched the two remaining dependency advisories, and the two "auto-dismissed" ones as well.**
  `brace-expansion` → **5.0.9** / **1.1.18** and `nanoid` → **3.3.18**. The brace-expansion pair is one chain:
  CVE-2026-14257 capped the *number* of expansions but not their *length*, so `'{a,b}'.repeat(1500)` still
  exhausted memory; CVE-2026-69152 then bypassed that mitigation through unbounded intermediate arrays. The fix
  adds a character cap on the accumulator. `nanoid` (CVE-2026-67213) loops forever when a custom generator is
  called with `size: 0`; it reaches us through `postcss`, which pins `^3.3.11` and has shipped no release moving
  that floor.
- **Retracted: the "known residual" this changelog declared in 0.9.1 was wrong by now.** That entry said no
  patched `brace-expansion` v1 existed, so `eslint-plugin-react` → `minimatch@3` had to stay vulnerable. Upstream
  since started maintaining the v1 line on the `maintenance-v1` dist-tag, and 1.1.17 / 1.1.18 keep the CommonJS
  `module.exports = expandTop` shape — verified against the published tarball, and a green `eslint .` is the
  practical proof. The v1 side is now genuinely patched rather than tolerated, and Constella carries **zero**
  open dependency advisories.
- **The Grok agent's filesystem bridge now uses the real workspace jail — it wasn't.** CodeQL flagged five
  `js/path-injection` alerts in `runGrok`, and behind them was an actual hole: the local containment check was
  *lexical only*. `fs-workspace.ts` has carried a proper guard for a long time — the same lexical check **plus** a
  realpath check on the nearest existing ancestor, written precisely to stop a prompt-injected agent from planting
  a symlink inside its own workspace and reading through it — and `runGrok` simply wasn't calling it. It is now:
  the ACP `fs/read_text_file` / `fs/write_text_file` handlers go through new `readContained` / `writeContained`
  helpers, so the filesystem calls sit *behind* the guard instead of at the call site. The wire protocol is
  unchanged (`-32602` refused, `-32000` I/O failure). `runGrok` also asserts the org id itself before building the
  spawn cwd rather than relying on `orgRoot()` one frame down, and `/api/locks/acquire` + `/api/sync/file` (both
  already worker-secret guarded) now return a clean 400 for a malformed `orgId` instead of an unhandled throw.
- **Two Windows path shapes the old check quietly allowed** turned up while testing the extracted helper and are
  now refused explicitly: a *drive-relative* path (`C:notes.txt` — `isAbsolute()` says false, so it was joined
  under the workspace root as a literal colon segment) and a path on *another drive* (`relative()` returns an
  absolute path there, not `../..`, so `join()` buried it under the root as nonsense). Secret scanning was and
  remains clean.

### Changed
- **The cloud model catalog is refreshed to 2026-09-07 — it had drifted past retirement, not just past currency.**
  Model ids that no longer resolve were still being offered: `gpt-5-codex` (shut down 2026-07-23) was the *first*
  entry in the Codex CLI picker and server allowlist, `grok-code-fast-1` (retired 2026-08-15) was in the Grok Build
  picker, and `grok-4` / `grok-3` (retired 2026-05-15) plus `gemini-3-pro` (its real slug `gemini-3-pro-preview` is
  shut down) were in the offline fallback. A retired slug either errors or silently reroutes to a model priced
  differently, so these are **deleted**, not demoted — and the fallback table now says so in its own header.
- **Added, with dates and prices taken from each vendor's own docs:** Claude **Fable 5.1** and **Sonnet 5**;
  **GPT-6 Astra** and the **GPT-5.6** family (Sol / Terra / Luna); **Gemini 3.8 / 3.7 / 3.5 Flash** and
  **3.1 Pro**; **Grok 4.6** and **Grok Build 0.1**. Anthropic output limits corrected from 64K to **128K**, and
  Sonnet's price to **$2/$10**. The Gemini 3.6–3.8 Flash prices are promotional through 2026-12-31 and double
  afterwards — noted inline so the next refresh catches it.
- **Opus 5 stays the recommended Anthropic default.** Fable 5.1 is the more capable model, but it is Opus-tier ×2
  on both sides ($10/$50 vs $5/$25); a new company should not silently staff its whole roster at that price. It is
  in the catalog and every picker — choose it per agent in Agent Studio.
- **Existing agents are untouched and there is no migration.** An agent's model lives on its own row (default
  `sonnet`), and the seeded roster uses CLI *aliases*, not ids — so `sonnet` simply resolves to Sonnet 5 and
  `fable` to Fable 5.1 the next time the Claude CLI runs.

### Fixed
- **Every HTTP-provider run was booking `usd: 0`.** The last-resort price table in `http.ts` still listed only
  `gpt-4o` and `gemini-1.5`, so no current model matched it. Refreshed, demoted in its comment to what it actually
  is (a fallback behind the models.dev catalog), and matching changed to longest-prefix so key order stops being
  silently load-bearing.
- **Grok agents were compacting at the unknown-model budget.** `modelWindow()` had no `grok` branch, so they fell
  through to the 100K/aggressive default; `fable` did too. Both are now recognised.
- **`npm run build:release` was failing before this release, and now doesn't.** `detectCliAuth` computed
  `const home = homedir()` and joined it with a runtime name. Next's file tracer (`@vercel/nft`) evaluates
  `homedir()` statically, so joining that known base with an unknown segment makes it give up and trace
  `<home>/**/*` — the build globs the operator's **entire home directory**. Slow everywhere, and on Windows it
  fails the build outright: the legacy `Meus Documentos` / `My Documents` junction is deny-listed, `scandir`
  returns `EPERM`, and it surfaces as a bare webpack compile error with no hint of where it came from. Reading
  `USERPROFILE`/`HOME` before falling back to `homedir()` is the same order `os.homedir()` uses internally, so
  the value is identical — the base just stops being a compile-time constant. Verified by tracing every glob the
  build issues: the `C:\Users\<user>/**/*` pattern is gone and the rest are bounded. There is no config-level
  alternative — `outputFileTracing: false` was removed in Next 16, and `outputFileTracingExcludes` filters
  results only after the glob has already run.
- **The MODELS.md CLI table documented two adapters that no longer exist.** `cli_openclaw` and `cli_hermes` were
  retired into `HIDDEN_CLI_ADAPTERS` and have carried no `CLI_MODELS` entry since 0.7, but the table still listed
  models for them. Both rows are gone, and the two notes that would have prevented this drift are in: the
  `cli_claude_code` entries are CLI *aliases* rather than model ids, and `CLI_MODELS` / `CLI_MODEL_OPTIONS` are
  hand-mirrored and must be edited together, in the same order.

### Known limitations
- **The local model catalog (GGUF + Ollama) was not refreshed this cycle.** Every entry needs an HTTP HEAD check
  against HuggingFace before it ships, or the download links rot silently — that is its own pass.
- **`modelWindow()`'s budgets stay far below the real context windows** (200K/128K against models that are now
  500K–1M). That number is the *compaction trigger*, so raising it means carrying ~5× more history into every
  call. That is a spend decision, not a data refresh, and it is deliberately not being made here.
- **`http.ts`'s price table is still a stub.** Replacing it with a live catalog lookup is the right fix; this
  release only stops it from being wrong about every model.
- **`skills/design/uupm-design` pins `gemini-3-pro-image-preview` / `gemini-3.1-pro-preview`.** Those are inside a
  bundled skill rather than the provider layer, and were left alone — but `-preview` image slugs are exactly the
  kind that get shut down, so they are worth a follow-up.

## [0.9.1] — "Patched dependencies + Claude Opus 5"

### Added
- **Claude Opus 5 is now the recommended default model.** The `opus` alias that Claude Code agents already
  use resolves to Opus 5, so the pickers now read "Claude Opus 5" instead of 4.8, and Anthropic's default
  preference leads with it (previously Sonnet). Opus 5 was added to the offline model catalog alongside 4.8 /
  4.7. Existing agents keep whatever model they were hired with — the seeded roster still assigns per role
  (Haiku for the light Docs/Knowledge agents, Sonnet mid, Opus for the critical ones) so a new company doesn't
  silently run its whole staff at Opus pricing; change any agent's model in Agent Studio.

### Security
- **Patched every actionable dependency advisory (7 high, 5 moderate).** `adm-zip` 0.5.17 → **0.6.0** (a ~120-byte
  crafted ZIP declaring a ~4 GB uncompressed size forced an unbounded `Buffer.alloc` before CRC validation —
  33-million-to-1 amplification, instant process crash; it reaches every archive path we expose, including goal
  restore and local-model extraction). `next` 16.2.9 → **16.2.11** (proxy/middleware bypass, Server Actions DoS,
  and SSRF on custom servers). `postcss` → **8.5.23**, `sharp` → **0.35.3** (libvips CVEs; pulled in only as an
  optional Next dep — we never render `<Image/>`), and `brace-expansion` to **5.0.8** / **1.1.16** (`expand()`
  ran in O(2ⁿ) over consecutive non-expanding `{}` groups: ~90 bytes of ASCII stalls the single-threaded event
  loop for minutes). The two `brace-expansion` major lines are pinned separately and caret-bounded on purpose —
  an open `>=` floor is satisfied across majors and silently hands v1 consumers the v5 API, which is
  `type: module` and exports an object where they call a function.
- **Known residual (dev-only, unfixable upstream):** GHSA-mh99-v99m-4gvg declares its only patched release to be
  `>=5.0.8`, so no fixed `brace-expansion` v1 exists, and `eslint-plugin-react` (already on its newest release)
  still pins `minimatch@^3.1.2`. It is deliberately not force-upgraded, since v5 would break that consumer.
  devDependencies never reach the published package, and ESLint only expands glob patterns from our own config —
  never untrusted input.

### Documentation
- **The Grok Build CLI adapter is now in the docs.** `cli_grok` was shipped in 0.9.0 but never reached the
  adapter tables in `MODELS.md` / `AI_ARCHITECTURE.md` — the only "grok" there was the unrelated HTTP `xai_grok`
  REST provider. Both tables now carry it, the `cli_claude_code` rows list the `fable` alias they were missing,
  and the recommended-default section reflects the new Anthropic preference order.
