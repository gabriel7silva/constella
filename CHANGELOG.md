...
- **The Design module no longer has its own chat column — you talk to Grace in her DM (AGENTS dock), and
  ...
  shown both in the dock and on the `/design` canvas, which paints live as she works. Opening `/design`
  auto-opens her DM alongside the canvas; picking a canvas element opens her DM scoped to that element; the
  Stop button in the DM cancels an in-flight build. The old chat column is retired (the left strip opens her
  DM); the canvas, Screens/Docs/Versions/Styles rail, comments and approve/handoff flow are unchanged.
