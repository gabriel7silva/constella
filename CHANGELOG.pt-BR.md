...
### Corrigido
- **O guia interno de release estava indo dentro do tarball público do npm.** O
  `scripts/publish-public.mjs` sempre excluiu `docs/en/PUBLISHING.md` / `docs/pt/PUBLISHING.md` do mirror
  **Git** público — é o guia interno e nomeia o remote privado de desenvolvimento — mas o canal do **npm**
  nunca teve a exclusão correspondente. O `files[]` lista `docs`, e isso é uma entrada de allowlist, então o
  npm empacotava a árvore inteira. Todo tarball publicado até a 0.9.2 inclusive levava os dois arquivos
  (verificado com `npm pack constellai@0.9.2`); não havia credencial neles, então não há nada para rotacionar
  — os dois canais só precisam concordar. Duas negações no `files[]` resolvem, e o tarball cai de 2001 para
  1999 arquivos. As negações precisam ficar logo abaixo de `docs`: uma negação em `files` só se aplica a um
  include que vem antes dela.

## [0.9.2] — "Todos os alertas de segurança fechados + a linha de modelos de 2026-09"

### Segurança
- **Corrigidas as duas vulnerabilidades de dependência abertas — e também as duas "auto-dispensadas".**
  `brace-expansion` → **5.0.9** / **1.1.18** e `nanoid` → **3.3.18**. O par do brace-expansion é uma corrente só:
  a CVE-2026-14257 limitou a *quantidade* de expansões mas não o *tamanho* delas, então 1500 grupos `{a,b}`
  seguidos ainda esgotavam a memória; a CVE-2026-69152 então contornou essa mitigação por meio de arrays
  intermediários sem limite. A correção adiciona um teto de caracteres no acumulador. O `nanoid`
  (CVE-2026-67213) entra em loop infinito quando um gerador customizado é chamado com `size: 0`; chega até nós
  pelo `postcss`, que fixa `^3.3.11` e não lançou nenhuma versão que suba esse piso.
- **Retratação: o "resíduo conhecido" que este changelog declarou na 0.9.1 já não era verdade.** Aquela entrada
  dizia que não existia `brace-expansion` v1 corrigida, então o `eslint-plugin-react` → `minimatch@3` teria de
  continuar vulnerável. Desde então o upstream passou a manter a linha v1 na dist-tag `maintenance-v1`, e a
  1.1.17 / 1.1.18 preservam o formato CommonJS `module.exports = expandTop` — verificado no tarball publicado, e
  um `eslint .` verde é a prova prática. O lado v1 agora está de fato corrigido, não apenas tolerado, e a
  Constella carrega **zero** vulnerabilidades de dependência em aberto.
- **A ponte de arquivos do agente Grok agora usa a jaula real do workspace — antes não usava.** O CodeQL apontou
  cinco alertas `js/path-injection` no `runGrok`, e atrás deles havia um buraco de verdade: a checagem local de
  contenção era *só léxica*. O `fs-workspace.ts` já carregava uma guarda apropriada há tempos — a mesma checagem
  léxica **mais** uma verificação de realpath no ancestral existente mais próximo, escrita justamente para
  impedir que um agente vítima de prompt injection plante um symlink dentro do próprio workspace e leia através
  dele — e o `runGrok` simplesmente não a chamava. Agora chama: os handlers ACP `fs/read_text_file` /
  `fs/write_text_file` passam pelos novos helpers `readContained` / `writeContained`, então as chamadas de
  filesystem ficam *atrás* da guarda, não no ponto de chamada. O protocolo na rede não muda (`-32602` recusado,
  `-32000` falha de I/O). O `runGrok` também valida o id da org ele mesmo antes de montar o cwd do spawn, em vez
  de depender do `orgRoot()` um frame abaixo, e `/api/locks/acquire` + `/api/sync/file` (ambos já protegidos pelo
  segredo do worker) passam a devolver um 400 limpo para um `orgId` malformado em vez de estourar sem tratamento.
- **Dois formatos de caminho do Windows que a checagem antiga deixava passar em silêncio** apareceram ao testar o
  helper extraído e agora são recusados explicitamente: caminho *relativo ao drive* (`C:notes.txt` — o
  `isAbsolute()` responde false, então ele era juntado sob a raiz do workspace como um segmento literal com
  dois-pontos) e caminho em *outro drive* (ali o `relative()` devolve um caminho absoluto, não `../..`, então o
  `join()` o enterrava sob a raiz como algo sem sentido). O secret scanning estava e continua limpo.

### Alterado
- **O catálogo de modelos cloud foi atualizado para 2026-09-07 — ele tinha ficado defasado até depois da
  aposentadoria dos modelos, não só desatualizado.** Ids que não resolvem mais continuavam sendo oferecidos: o
  `gpt-5-codex` (desligado em 2026-07-23) era a *primeira* entrada do seletor e da allowlist da Codex CLI, o
  `grok-code-fast-1` (aposentado em 2026-08-15) estava no seletor do Grok Build, e `grok-4` / `grok-3`
  (aposentados em 2026-05-15) mais `gemini-3-pro` (cujo slug real `gemini-3-pro-preview` foi desligado) estavam
  no fallback offline. Um slug aposentado ou dá erro ou é redirecionado em silêncio para um modelo com outro
  preço, então esses foram **removidos**, não rebaixados — e a tabela de fallback agora diz isso no próprio
  cabeçalho.
- **Adicionados, com datas e preços tirados da documentação de cada fornecedor:** Claude **Fable 5.1** e
  **Sonnet 5**; **GPT-6 Astra** e a família **GPT-5.6** (Sol / Terra / Luna); **Gemini 3.8 / 3.7 / 3.5 Flash** e
  **3.1 Pro**; **Grok 4.6** e **Grok Build 0.1**. Os limites de saída da Anthropic foram corrigidos de 64K para
  **128K**, e o preço do Sonnet para **$2/$10**. Os preços dos Gemini 3.6–3.8 Flash são promocionais até
  2026-12-31 e dobram depois — anotado no código para a próxima atualização pegar.
- **O Opus 5 continua o padrão recomendado da Anthropic.** O Fable 5.1 é o modelo mais capaz, mas custa o dobro
  do tier Opus dos dois lados ($10/$50 contra $5/$25); uma empresa nova não deve rodar o quadro inteiro nesse
  preço sem querer. Ele está no catálogo e em todos os seletores — escolha por agente no Agent Studio.
- **Agentes existentes não são tocados e não há migração.** O modelo de um agente vive na linha dele (padrão
  `sonnet`), e o time inicial usa *aliases* de CLI, não ids — então `sonnet` simplesmente resolve para o Sonnet 5
  e `fable` para o Fable 5.1 na próxima execução da CLI do Claude.

### Corrigido
- **Toda execução de provedor HTTP registrava `usd: 0`.** A tabela de preços de último recurso do `http.ts` ainda
  listava só `gpt-4o` e `gemini-1.5`, então nenhum modelo atual batia com ela. Foi atualizada, rebaixada no
  comentário ao que de fato é (um fallback atrás do catálogo do models.dev), e o casamento passou a ser por
  prefixo mais longo, para a ordem das chaves parar de importar em silêncio.
- **Agentes Grok compactavam no orçamento de modelo desconhecido.** O `modelWindow()` não tinha ramo para
  `grok`, então caíam no padrão de 100K/agressivo; o `fable` também. Ambos passam a ser reconhecidos.
- **O `npm run build:release` já estava quebrado antes deste release, e agora não está mais.** O
  `detectCliAuth` calculava `const home = homedir()` e juntava isso com um nome vindo em tempo de execução. O
  tracer de arquivos do Next (`@vercel/nft`) avalia o `homedir()` estaticamente, então juntar essa base
  conhecida com um segmento desconhecido faz ele desistir e rastrear `<home>/**/*` — o build varre o diretório
  **inteiro** do usuário. Lento em qualquer sistema, e no Windows quebra o build de vez: a junção legada
  `Meus Documentos` / `My Documents` é negada por ACL, o `scandir` devolve `EPERM`, e isso aparece como um erro
  cru de compilação do webpack sem nenhuma pista da origem. Ler `USERPROFILE`/`HOME` antes de cair no
  `homedir()` é a mesma ordem que o próprio `os.homedir()` usa por dentro, então o valor é idêntico — a base
  apenas deixa de ser uma constante de compilação. Verificado rastreando todos os globs que o build dispara: o
  padrão `C:\Users\<usuário>/**/*` sumiu e os demais são todos limitados. Não há alternativa via configuração —
  o `outputFileTracing: false` foi removido no Next 16, e o `outputFileTracingExcludes` filtra os resultados só
  depois que o glob já rodou.
- **A tabela de CLIs do MODELS.md documentava dois adaptadores que não existem mais.** `cli_openclaw` e
  `cli_hermes` foram aposentados para `HIDDEN_CLI_ADAPTERS` e não têm entrada em `CLI_MODELS` desde a 0.7, mas a
  tabela ainda listava modelos para eles. As duas linhas saíram, e entraram as duas notas que teriam evitado essa
  defasagem: as entradas de `cli_claude_code` são *aliases* de CLI, não ids de modelo, e `CLI_MODELS` /
  `CLI_MODEL_OPTIONS` são espelhados à mão e precisam ser editados juntos, na mesma ordem.

### Limitações conhecidas
- **O catálogo de modelos locais (GGUF + Ollama) não foi atualizado neste ciclo.** Cada entrada precisa de uma
  checagem HTTP HEAD no HuggingFace antes de ir pro ar, senão os links de download apodrecem em silêncio — isso é
  uma passada própria.
- **Os orçamentos do `modelWindow()` continuam bem abaixo das janelas de contexto reais** (200K/128K contra
  modelos que hoje são de 500K–1M). Esse número é o *gatilho de compactação*, então aumentá-lo significa carregar
  ~5× mais histórico em toda chamada. Isso é uma decisão de gasto, não uma atualização de dados, e de propósito
  não está sendo tomada aqui.
- **A tabela de preços do `http.ts` ainda é um paliativo.** Substituí-la por uma consulta ao catálogo ao vivo é a
  correção certa; este release só impede que ela esteja errada sobre todos os modelos.
- **`skills/design/uupm-design` fixa `gemini-3-pro-image-preview` / `gemini-3.1-pro-preview`.** Esses estão
  dentro de uma skill empacotada, não na camada de provedores, e ficaram como estavam — mas slugs de imagem
  `-preview` são exatamente os que costumam ser desligados, então valem um acompanhamento.

## [0.9.1] — "Dependências corrigidas + Claude Opus 5"

### Adicionado
- **O Claude Opus 5 agora é o modelo padrão recomendado.** O alias `opus` que os agentes do Claude Code já usam
  resolve para o Opus 5, então os seletores passam a mostrar "Claude Opus 5" no lugar do 4.8, e a preferência
  padrão da Anthropic começa por ele (antes era o Sonnet). O Opus 5 foi adicionado ao catálogo offline de modelos
  junto do 4.8 / 4.7. Agentes existentes mantêm o modelo com que foram contratados — o time inicial continua sendo
  distribuído por papel (Haiku para os agentes leves de Docs/Conhecimento, Sonnet no meio, Opus nos críticos) pra
  que uma empresa nova não rode o quadro inteiro no preço do Opus sem querer; troque o modelo de qualquer agente
  no Agent Studio.

### Segurança
- **Corrigidas todas as vulnerabilidades acionáveis das dependências (7 altas, 5 moderadas).** `adm-zip` 0.5.17 →
  **0.6.0** (um ZIP forjado de ~120 bytes declarando ~4 GB de tamanho descomprimido forçava um `Buffer.alloc` sem
  limite antes da validação de CRC — amplificação de 33 milhões pra 1, crash imediato do processo; atinge todos os
  caminhos de arquivo compactado que expomos, incluindo restaurar meta e extrair modelos locais). `next` 16.2.9 →
  **16.2.11** (bypass de proxy/middleware, DoS nas Server Actions e SSRF em servidores customizados). `postcss` →
  **8.5.23**, `sharp` → **0.35.3** (CVEs do libvips; entra só como dep opcional do Next — nunca renderizamos
  `<Image/>`), e `brace-expansion` para **5.0.8** / **1.1.16** (o `expand()` rodava em O(2ⁿ) sobre grupos `{}`
  consecutivos não-expansíveis: ~90 bytes de ASCII travam o event loop single-threaded por minutos). As duas
  linhas major do `brace-expansion` são fixadas separadamente e limitadas por caret de propósito — um piso `>=`
  aberto é satisfeito entre majors e entrega silenciosamente a API v5 pra quem consome a v1, que é `type: module`
  e exporta um objeto onde eles chamam uma função.
- **Resíduo conhecido (só de desenvolvimento, sem correção upstream):** a GHSA-mh99-v99m-4gvg declara que a única
  versão corrigida é `>=5.0.8`, ou seja, não existe `brace-expansion` v1 corrigida, e o `eslint-plugin-react` (já
  na versão mais recente) ainda fixa `minimatch@^3.1.2`. Não forçamos o upgrade de propósito, porque a v5 quebraria
  esse consumidor. devDependencies nunca chegam ao pacote publicado, e o ESLint só expande globs da nossa própria
  config — nunca entrada não-confiável.

### Documentação
- **O adaptador CLI do Grok Build entrou na documentação.** O `cli_grok` foi lançado na 0.9.0 mas nunca chegou às
  tabelas de adaptadores do `MODELS.md` / `AI_ARCHITECTURE.md` — o único "grok" ali era o provedor HTTP `xai_grok`
  (a API REST), que é outra coisa. Agora as duas tabelas têm ele, as linhas do `cli_claude_code` listam o alias
  `fable` que faltava, e a seção de padrão recomendado reflete a nova ordem de preferência da Anthropic.

## [0.9.0] — "Agentes Grok Build (CLI da xAI)"

### Adicionado
- **Grok Build — o agente de código nativo de terminal da xAI — agora é um adaptador CLI contratável.** O Grok
  Build (com Grok 4.5) roda headless pela interface ACP (Agent Client Protocol) JSON-RPC via stdio (`grok agent
  stdio`) — o prompt viaja no corpo JSON pelo stdin (sem limite de linha de comando) e edita arquivos + roda
  shell como os outros CLIs agentic; a Constella atende os pedidos de leitura/escrita de arquivo + permissão
  dele (auto-aprovados, presos ao workspace). É
  auto-detectado na máquina no onboarding, aparece no catálogo de provedores e nos seletores de modelo do
  Hire / Agent Studio (modelos: `grok-code-fast-1`, `grok-4.5`, `grok-build-0.1`), e autentica pelo sign-in do
  `grok` no browser (SuperGrok / X Premium+) ou pela env `GROK_CODE_XAI_API_KEY`. Instalar:
  `curl -fsSL https://x.ai/cli/install.sh | bash` (ou `irm https://x.ai/cli/install.ps1 | iex` no Windows). O CLI
  não emite token/custo, então ficam registrados como 0 (honesto). Diferente do provedor HTTP `xAI / Grok`
  existente (a API REST) — este é o CLI do agente local.

## [0.8.0] — "Um chat só da Grace: o DM dela constrói o canvas"

### Mudado
- **O módulo Design não tem mais coluna de chat própria — você fala com a Grace no DM dela (dock AGENTES), e
  esse DM constrói/atualiza o canvas.** Antes existiam duas conversas separadas com a Grace (o chat do módulo
  Design no canal `design` + um DM genérico `dm:grace`), o que era redundante. Agora o DM da Grace roda no mesmo
  canal `design` e cada envio dispara a build real do protótipo (`askDesign`) — um thread só, mostrado tanto no
  dock quanto no canvas do `/design`, que pinta ao vivo enquanto ela trabalha. O chat do Design foi
  **movido fisicamente** pra dentro do DM dela (um `design-chat.tsx` self-contained) com comportamento
  **idêntico** — streaming token a token, chips de sugestão, anexar/colar/arrastar imagem, elemento-anexado do
  canvas, donut de contexto + compactar, Parar — e removido do módulo (uma faixa fina à esquerda abre o DM).
  Abrir `/design` já abre o DM ao lado do canvas; escolher um elemento no canvas abre o DM focado nele. O canvas,
  o rail de Telas/Docs/Versões/Estilos, os comentários e o fluxo de aprovar/handoff continuam iguais.

### Corrigido
- **As respostas da Grace no Design apareciam como `(couldn't respond: <a resposta real dela>)` e podiam vir
  cortadas.** Numa build longa o run do CLI terminava não-limpo (limite de turnos / timeout de 10min) mesmo tendo
  produzido o resumo final real dela — o código tratava isso como falha e embrulhava a resposta real no template
  de erro. Agora o texto da resposta é entregue sempre que existe (o template `(couldn't respond …)` fica só pro
  caso de falha sem saída nenhuma), o timeout da build de design sobe pra 20min pra builds longas terminarem em
  vez de cortarem, e o `subtype` do resultado é exposto pra diagnóstico. Mesmo fix no run de docs do handoff e no
  caminho de resposta da Sala do Time/DM/Telegram.

## [0.7.4] — "O worker alcança os próprios endpoints"

### Corrigido
- **Os endpoints do worker eram redirecionados pro `/login`, quebrando em silêncio o tick 24/7, o poll de
  entrada do Telegram e o sync disco→DB.** O proxy de sessão (`src/proxy.ts`) só isentava as rotas de auth +
  health, então os `POST` sem cookie do worker pra `/api/cron/tick`, `/api/telegram/poll` e `/api/sync/file`
  levavam um redirect 307 pro `/login`; o `fetch` do worker seguia o redirect, pegava o HTML do login, e o
  `res.json()` virava `{}` — logando um enganoso `tick 200 {}` enquanto nada rodava de fato. Sintoma que o
  operador viu: **mensagens mandadas pro bot pelo Telegram nunca chegavam no Constella** (as respostas de
  saída funcionavam, pois não passam pelo worker). Essas quatro rotas já se autenticam com
  `CONSTELLA_WORKER_SECRET` (fail-closed 401 sem ele), então agora são isentas do portão de sessão — o secret
  segue protegendo. Cobre também a rota do lock-hook dos agentes `/api/locks/acquire`. Mais visível em deploy
  de rede (`--vps`), onde worker e navegador são mesma origem mas o worker não carrega sessão.
