...
- **O módulo Design não tem mais coluna de chat própria — você fala com a Grace no DM dela (dock AGENTES), e
  ...
  dock quanto no canvas do `/design`, que pinta ao vivo enquanto ela trabalha. Abrir `/design` já abre o DM dela
  ao lado do canvas; escolher um elemento no canvas abre o DM focado nesse elemento; o botão Parar no DM cancela
  a build em andamento. A antiga coluna de chat foi aposentada (a faixa à esquerda abre o DM); o canvas, o rail
  de Telas/Docs/Versões/Estilos, os comentários e o fluxo de aprovar/handoff continuam iguais.
